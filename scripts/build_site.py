...
DAY_END = "<!-- DAYS:END -->"
INTRO_BEGIN = "<!-- INTRO:BEGIN — generated by scripts/build_site.py, do not hand-edit -->"
INTRO_END = "<!-- INTRO:END -->"
...
# the commons itself — so it gets its own strip above the five track cards and is
# excluded from the counts on them. Naming the track here rather than inferring
# it means a renumber that moves the introduction fails the build loudly.
INTRO_TRACK = "A0"


def intro_block() -> str:
    """The introduction chapter, one link per lesson, above the track cards.

    Every reader hits this before choosing a function, and the whole reason it
    exists is the feedback that nobody could tell what the commons was for until
    somebody explained it. Burying it inside Function A's card was the version
    of that mistake the homepage was making.
    """
    track = next((t for f in CUR["functions"] for t in f["tracks"]
                  if t["id"] == INTRO_TRACK), None)
    if track is None:
        raise SystemExit(f"scripts/build_site.py: no track {INTRO_TRACK} — the "
                         f"homepage introduction strip is generated from it")
    out = ['<div class="intro rv">']
    for s in track["sessions"]:
        out.append(f'<a class="ilesson" href="lessons/{s["id"]}.html">'
                   f'<span class="ik">{s["id"]}</span>'
                   f'<span class="it">{html.escape(s["title"])}</span></a>')
    out.append("</div>")
    return "\n".join(out)
...
def homepage(src: str) -> str:
    ...
    if INTRO_BEGIN not in src or INTRO_END not in src:
        raise SystemExit("site/index.html has lost its INTRO markers")
    head, rest = src.split(INTRO_BEGIN, 1)
    _, tail = rest.split(INTRO_END, 1)
    src = f"{head}{INTRO_BEGIN}\n{intro_block()}\n{INTRO_END}{tail}"

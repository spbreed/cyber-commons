...
def days_block() -> str:
    """Who each track is for, and its Day 0 / Day 1 / Day 2.

    Readers kept saying they could not tell what the commons was for until
    somebody explained it. This is that explanation, generated from the same
    `FUNCTION_DAYS` the lesson pages render, so the homepage cannot describe a
    curriculum the lessons do not deliver.
    """
    from exercises.days import FUNCTION_DAYS
    missing = [f["id"] for f in CUR["functions"] if f["id"] not in FUNCTION_DAYS]
    if missing:
        raise SystemExit(f"scripts/build_site.py: no Day 0/1/2 for function(s) "
                         f"{missing} — add them to scripts/exercises/days.py")
    out = ['<div class="days rv">']
    for i, fn in enumerate(CUR["functions"], 1):
        d = FUNCTION_DAYS[fn["id"]]
        name = TRACKS[fn["id"]][0]
        first = fn["tracks"][0]["sessions"][0]["id"]
        out.append(
            f'<div class="dayrow">'
            f'<div class="dayhd"><div class="k">{i:02d}</div>'
            f'<h3><a href="lessons/{first}.html">{name}</a></h3>'
            f'<p class="who"><b>Who</b> · {html.escape(d["who"])}</p></div>'
            f'<div class="daycols">'
            f'<div><span class="dtag d0">Day 0 · why</span>'
            f'<p>{html.escape(d["day0"])}</p></div>'
            f'<div><span class="dtag d1">Day 1 · how</span>'
            f'<p>{html.escape(d["day1"])}</p></div>'
            f'<div><span class="dtag d2">Day 2 · measure</span>'
            f'<p>{html.escape(d["day2"])}</p></div>'
            f'</div></div>')
    out.append("</div>")
    return "\n".join(out)

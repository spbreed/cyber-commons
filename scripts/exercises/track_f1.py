"""F1 — The GRC Practitioner (Risk & Control). Nine sessions.
...
and none of the changes that break it are code changes.
    ...
    F1.1  why point-in-time testing fails
    F1.2  the inventory — you cannot govern what you cannot list
    F1.3  risk tiering by authority and data, not by model
    F1.4  control mapping (control → framework, never the reverse)
    F1.5  evaluation output as audit evidence
    F1.6  operating vs outcome guardrails
    F1.7  continuous control verification
    F1.8  third-party and model supply chain

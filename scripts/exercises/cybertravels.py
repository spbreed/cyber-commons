...
# --------------------------------------------------------------------------
# One or two sentences per lesson, tying its idea to TripBot. Rendered as a
...
"A1.0": "TripBot is the system. Everything after this lesson names one of its "
        "boxes.",
...
"A1.6": "TripBot inherits Alex's standing permissions because that was the "
        ...
        "anything it does. R1.",
"A1.7": "All four agents share one service account, `tripbot-svc`. The payments "
        "API can see that TripBot called it and cannot see which of the four — "
...
"A1.9": "The Workflow Agent asks the RAG Advisor for a hotel summary. The reply "
        ...
        "would. R3.",
"A1.10": "TripBot's orchestrator delegates to the agents it discovers. A fifth "
...
"A1.13": "The log says `tripbot-svc issued refund 8812`. It does not say which "
...
"A1.14": "Approval on every refund is a real control at four a day. TripBot "
...
"A1.16": "An employee who cannot issue refunds asks TripBot to, and it can. And "
...
"A2.1": "Three identities are present whenever TripBot books a flight: the "
        ...
        "`tripbot-svc`, which is R11.",
"A2.2": "Each of TripBot's four agents needs a credential to prove it is that "
...
"A2.3": "Alex can issue refunds; the triage agent must never be able to. "
        "Delegation from Alex to TripBot has to narrow to a subset of what he "
...
"A2.5": "`tripbot-svc` was created for a proof of concept in March. The proof "
...
"A2.7": "The four fields an auditor will ask about that $5,000 refund: which "
        ...
        "act. TripBot currently records the third and a version of the second. "
        "R11.",
"A2.8": "If the Coding Agent can write to the log store, then every detection "
        "CyberTravels builds on TripBot's transcripts is a conclusion about the "
...
"A3.3": "Every way customer PII leaves CyberTravels — a prompt leak, an abused "
        ...
        "boundary. R9, R10.",
"A3.4": "What makes “TripBot runs autonomously” a bounded sentence: a ceiling "
...
"A3.9": "Alex turned the guardrails off for the demo. That was defensible. What "
        "was never decided is what TripBot's blast radius should have shrunk to "
        "while they were off. R2.",
"A3.10": "TripBot's tool list has `book_flight`, `issue_refund` and "
...
"B1.1": "TripBot's repository is four services and a shared library. Structure "
        "is what decides whether the pipeline reads the booking handlers or the "
        "vendored SDK first.",
"B1.2": "Every later stage inherits this map. Summarise the payments service "
        "wrongly and the threat model misses the refund path — which is R1 "
        "arriving through the review pipeline rather than through the agent.",
"B1.3": "The threat model that said “TripBot answers questions” is still on "
        "file. Deriving it from the architecture on every release is what would "
        "have caught the refund endpoint appearing.",
"B1.4": "CyberTravels has finite review budget and four services. Spending it "
        "on the payments path rather than the vendored SDK is the highest-"
        "leverage decision in the whole pipeline.",
"B1.5": "The IDOR that exposed card details by booking ID (R8) is exactly the "
        "class each generation of SAST handles differently — and the class the "
        "third generation will also confidently invent.",
"B1.6": "Three analysers found the same booking-handler defect four times. The "
        "queue Alex will actually read is the deduplicated one.",
"B1.7": "The finding is real and the code is unreachable from any traveller "
        "input. Reachability is what stops CyberTravels' queue becoming "
        "something engineers learn to ignore.",
"B1.8": "You cannot confirm the IDOR by exploiting it in production. The replica "
        "is where the booking API can be attacked safely, and its fidelity "
        "decides which findings are confirmable at all.",
"B1.9": "A finding becomes a fact when something other than a model says so — "
        "here, a request to the replica's booking endpoint that returns another "
        "traveller's card details.",
"B1.10": "Verbose errors, an open redirect and a path traversal are each low on "
         "their own. Chained against TripBot they read a config file and end the "
         "conversation about severity. R9.",
"B1.11": "The Coding Agent's fix must not break booking behaviour. A patch that "
         "passes the tests and changes what travellers experience is a second "
         "incident with a pull request attached. R8.",
"B1.12": "CVSS scores the vulnerability. CyberTravels' engineers are asking "
         "about this booking API, with card data, behind this gateway — and the "
         "number that answers them is not on the badge.",
"B1.13": "Give the review agent TripBot's whole repository and it gets worse, "
         "not better. The cliff arrives earlier than anyone expects and you pay "
         "more per token for it.",
"B1.14": "The pipeline reads TripBot's code, and TripBot's code includes what "
...
"B1.15": "The Coding Agent on Alex's laptop holds repository write, a cloud "
         ...
         "R7.",
"B1.16": "“CyberTravels enforces least privilege on TripBot” is true of some "
         "deployment at some time. An attestation is what binds it to the one "
         "running now — and refuses to claim more than it can show.",
"B1.17": "Somebody else has already built this pipeline and published what "
...
# ---- B2 · the harness ----------------------------------------------------
"B2.0": "Before Alex can trust a finding about TripBot he has to be able to name "
...
"B2.4": "Reviewing all four TripBot services on the strongest model is how the "
...
"B2.9": "CyberTravels needs SAST, DAST, a threat model and a pentest against "
        "TripBot. They are one skeleton with four different oracles, and "
        "building four harnesses is how four teams each get the oracle wrong "
        "differently.",
"B2.10": "The vendor chart was not run on TripBot's repository, at "
         "CyberTravels' context length, with CyberTravels' tools.",
...
"B2.12": "At 80% per-run reliability, a review harness left to run unattended "
         "over TripBot's releases is right about a third of the time — and "
         "somebody at CyberTravels is paying per confirmed finding.",
"B2.13": "A canary credential in TripBot's environment and a honeypot task in "
         "the benchmark: two detectors with no threshold to tune, because "
         "nothing legitimate has any reason to touch either.",

# ---- C1 · red teaming ----------------------------------------------------
"C1.0": "The board asked whether TripBot is secure. This function answers the "
...
"C1.2": "One campaign across TripBot's three surfaces — the booking note it "
...
"C1.4": "The finding that TripBot refunds on request is worth nothing to Alex "
...
"C2.2": "“TripBot refunded a booking when I asked” is one attempt. The rate, "
...
"C2.6": "Change TripBot's model and its review harness in the same week and "
...
"C2.8": "Read as a story about an AI lab it is a curiosity. Read as a list of "
        "preconditions, eight of the nine are already true inside TripBot.",
...
"D2.1": "Reconstructing what TripBot did across six log sources is reading, and "
...
"D2.3": "Eleven minutes of TripBot on delegated credentials. What it touched is "
        "not answerable from memory — it comes out of the identity and egress "
        "logs, if they exist. R9.",
"D2.4": "You have to stop TripBot faster than it issues refunds. The containment "
...
"D2.8": "The disclosure clock started when TripBot exported the customer "
        "profiles, not when CyberTravels understood what had happened. Passport "
        "and payment data make the deadline short. R10.",
"D2.9": "Terminating TripBot's four agents while their bearer tokens stay valid "
        "moves the incident rather than ending it. R5.",

# ---- E1 · risk and control -----------------------------------------------
"E1.0": "Someone at CyberTravels signed off on TripBot when it was a chatbot. It "
        "now issues refunds, ships code and indexes contracts.",
"E1.1": "The control test that passed in March described a TripBot with no "
...
"E1.4": "Most of TripBot's risks map onto controls CyberTravels already has. "
        "Building a parallel AI control estate is the expensive mistake; finding "
        "the genuine gaps is the work.",
"E1.5": "The eval that says TripBot recommends real hotels 94% of the time is "
...
"E1.6": "Two different controls get confused: what bounds how TripBot runs "
...
"E2.1": "CyberTravels holds passports, payment data and health information for "
        "corporate travellers. Layers 2 and 3 were in force before TripBot "
        "existed.",
"E2.2": "Horizontal AI obligations land on TripBot as programme requirements: a "
        "risk register, technical documentation, named human oversight, measured "
        "accuracy, post-market monitoring.",
"E2.3": "One control set for TripBot, mapped outward to every regime that asks — "
...
"E2.4": "A travel company touches payment rules, privacy law and, through "
        ...
        "replace.",
"E2.5": "Passport numbers reach TripBot's prompts, its context window, its "
...
"E2.7": "The document describing TripBot's oversight has to survive a supervisor "
        "asking when that oversight last operated. R2.",
"E2.8": "“Why did TripBot issue that refund?” has a legal deadline attached. "
...
"E2.9": "A supervisor can tell the difference between confidence and evidence. "
        "Opening with TripBot's real numbers is the only version that survives "
        "the follow-up.",

# ---- E3 · the CISO office ------------------------------------------------
"E3.1": "The board does not want TripBot's threat model. It wants the exposure, "
...
"E3.3": "Everything in TripBot's programme depends on something else in it. "
...
"E3.7": "CyberTravels cannot hire an identity engineer, a detection engineer "
        "and a harness engineer at the rate TripBot is changing. Most of that "
        "capability has to be built.",
"E3.8": "TripBot will fail sometimes, because it is probabilistic. A programme "

...
E1.0 and F1.1 carry no anchor, because they define the unit. F1.0 carries none
...
"E1.3":
 "**Anchor → E1.0.** Both halves set the floor for all five intervals. An agent "
...
"E3.9":
 "**Anchor → E1.0.** Intelligence shortens the *detect* interval before the "
...
"E1.1":
 "**Anchor → E1.0.** Discover has a floor, and this lesson measures it. Four "
...
"E1.2":
 "**Anchor → E1.0.** Drift lengthens every interval at once, silently. The "
...
"E3.10":
 "**Anchor → E1.0.** Hunting is the only pass that shortens discover for "
...
# ---- E2 · detect — from visible to an alert -------------------------------
"E2.1":
 "**Anchor → E1.0.** Detect is bounded below by what the lake can answer and "
...
"E2.2":
 "**Anchor → E1.0.** This is the detect interval for the agent itself, and the "
...
"E2.3":
 "**Anchor → E1.0.** A different subject is a different clock. Workload-layer "
...
"E2.4":
 "**Anchor → E1.0.** This shortens the gap between *a rule is needed* and *a "
...
"E2.5":
 "**Anchor → E1.0.** The fastest route to a rule, and the one most likely to "
...
"E2.6":
 "**Anchor → E1.0.** The only detector whose interval is the moment of touch. "
...
# ---- E3 · investigate — from an alert to a conclusion ---------------------
"E3.1":
 "**Anchor → E1.0.** The understand interval stops being one analyst's reading "
...
"E3.2":
 "**Anchor → E1.0.** This one **spends** the interval on purpose. Bounding the "
...
"E3.3":
 "**Anchor → E1.0.** Acting identity, scopes held and the delegation chain are "
...
"E3.4":
 "**Anchor → E1.0.** Three responder instincts each burn the interval: "
...
"E3.5":
 "**Anchor → E1.0.** This shortens the interval and can end it in the wrong "
...
"E3.6":
 "**Anchor → E1.0.** An agent that scores evidence only on support never ends "
...
"E3.7":
 "**Anchor → E1.0.** Scope decides what *understood* covers. Following the last "
...
"E3.8":
 "**Anchor → E1.0.** For a coordinated fleet, per-run monitoring never ends the "
...
# ---- E4 · respond — from a conclusion to the actor stopped ----------------
"E4.1":
 "**Anchor → E1.0.** This fixes the contain interval **before** the incident, "
...
"E4.2":
 "**Anchor → E1.0.** Three tiers are three contain intervals at three different "
...
"E4.3":
 "**Anchor → E1.0.** The interval where the arithmetic is brutal. An agent at "
...
"E4.4":
 "**Anchor → E1.0.** This is the interval itself, measured end to end rather "
...
"E4.5":
 "**Anchor → E1.0.** Contain, when the unit is the fleet — and the detail that "
...
# ---- E5 · recover — from stopped to the control back at target ------------
"E5.1":
 "**Anchor → E1.0.** Recovery starts from a run you can reproduce. Miss one of "
...
"E5.2":
 "**Anchor → E1.0.** A root cause naming a person ends no interval, because "
...
"E5.3":
 "**Anchor → E1.0.** The fix lands at a layer, and the layers that pass through "
...
"E5.4":
 "**Anchor → E1.0.** This is where recover actually ends: an indicator back at "
...
"E5.5":
 "**Anchor → E1.0.** Recovery that changes only what is deployed leaves the "
...
"E5.6":
 "**Anchor → E1.0.** The one interval you neither own nor can shorten. It "
...
# ---- F1 · the framework that produces the indicators ---------------------
"F1.2":
 "**Anchor → F1.1.** The inventory is the **denominator**. Every indicator is "
...
"F1.3":
 "**Anchor → F1.1.** Risk tiering sets the **target**. The same indicator "
 "carries a different threshold on a tier-1 agent that moves money than on a "
 "tier-3 one that summarises documents, and F1.1's rule — the target is "
 "written before the measurement — is only meetable once the tier exists.",

"F1.4":
 "**Anchor → F1.1.** Control mapping is where an indicator gets its subject. "
 "A mapped control with no indicator behind it is the state F1.1 calls "
 "unevidenced: it is neither passing nor failing, and most GRC tooling will "
 "render it green.",

"F1.5":
 "**Anchor → F1.1.** An eval result **is** an indicator reading — the sample "
...
"F1.6":
 "**Anchor → F1.1.** Operating and outcome guardrails need different "
...
"F1.7":
 "**Anchor → F1.1.** This is the machinery that re-measures. F1.1 argues that "
...
"F1.8":
 "**Anchor → F1.1.** These are the indicators you **cannot compute yourself**. "
...
"F1.9":
 "**Anchor → F1.1.** A retrain, a fine-tune or a re-index **invalidates a "
...
"F1.10":
 "**Anchor → F1.1.** One indicator, five readers, five different questions. "
...
"F1.11":
 "**Anchor → F1.1.** Model risk management arrived at the same answer decades "
...
"F1.12":
 "**Anchor → F1.1.** The seams are where an indicator has two consumers and no "
...
"F1.13":
 "**Anchor → F1.1.** This is F1.1 executed. Six indicators, computed from the "
 "CyberTravels tree at run time, reported as gaps with named mitigations "
 "rather than as a score.",

# ---- F2 · the indicators as regulatory evidence ---------------------------
"F2.1":
 "**Anchor → F1.1.** The reason one control set can satisfy several regimes is "
...
"F2.2":
 "**Anchor → F1.1.** Horizontal regulation asks whether a system is robust, "
...
"F2.3":
 "**Anchor → F1.1.** The framework spine is where indicators **attach**. Pick "
...
"F2.4":
 "**Anchor → F1.1.** A sector overlay mostly re-targets indicators you "
...
"F2.5":
 "**Anchor → F1.1.** Privacy indicators measure paths rather than outcomes — "
...
"F2.6":
 "**Anchor → F1.1.** A disclosure clock runs on facts you either measured "
 "before the incident or are guessing at during it. E5.4 re-measures the "
 "indicators after the fix; this lesson is why the pre-incident reading has to "
 "have existed at all.",

"F2.7":
 "**Anchor → F1.1.** Documentation that survives supervision is largely "
...
"F2.8":
 "**Anchor → F1.1.** The delegation chain is an indicator in its own right: "
...
"F2.9":
 "**Anchor → F1.1.** A regulator conversation goes differently when bounded "
...
# ---- F3 · the indicators as a programme -----------------------------------
"F3.1":
 "**Anchor → F1.1.** Translating risk upward is mostly choosing which three "
...
"F3.2":
 "**Anchor → F1.1.** Governing autonomy rather than tools means the policy "
...
"F3.3":
 "**Anchor → F1.1.** The order is a dependency order, not a gap-size order, "
...
"F3.4":
 "**Anchor → F1.1.** Org design decides who can actually compute each "
...
"F3.5":
 "**Anchor → F1.1.** These **are** the indicators, at the level a CISO "
 "reports — exposure, likelihood, assurance, coverage, speed. The property "
 "that qualifies each of them is F1.1's: it is computed rather than assessed, "
 "so it degrades on its own. A number that stays flat under neglect is "
 "measuring activity.",

"F3.6":
 "**Anchor → F1.1.** Autonomy promotion as an earned event needs named "
...
"F3.7":
 "**Anchor → F1.1.** The build curve is unglamorous early precisely because "
...
"F3.8":
 "**Anchor → F1.1.** Resilience is measured by the containment, detection and "
 "recovery indicators — the same ones E5.4 re-reads after a fix. Prevention "

"""B2 — Controls: identity and ingress.

Chapter B1 named the risks. This chapter closes the two that everything else
...
exactly one block of code implementing it.
    ...
    B2.1  agent identity            T9, T3, T1
    B2.2  bootstrapping             T9
    B2.3  delegation that narrows   T3, T8, T14
    B2.4  just-in-time authority    T3, T2
    B2.5  the NHI lifecycle         T9, T13, T3
    B2.6  ingress provenance        T6, T1, T12
    B2.7  attribution               T8, T13
"""
...
""",
 "steps": [
  ("md", MITIGATES + "> Makes **possession stop being proof**. Without it, "
         "B1.7 is unavoidable: a copyable secret means every holder is the "
         "agent.\\n\\n## 2 · The control"),
...
deployment rather than assume the control is on because the feature exists."""),
  ("md", "## 5 · What the audit trail can now answer\n\nEvery hop is on the "
         ...
         "correlating four services' logs on timestamp. This is the thing B1.14 "
         "could not do."),
...
""",
 "steps": [
  ("md", MITIGATES + "> Turns 'which agents are allowed here' from a "
         ...
         "an audit question into a leaver event. Closes B1.11 at the door.\n\n"

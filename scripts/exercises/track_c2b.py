"""B1 (part 2) — The AppSec pipeline, phases 4 and 5, plus the bonus.
    ...
    Supply chain (pre-deployment, and it belongs before the replica)
       SBOM, dependency vulns, decompiling the undeclared         → C2.7
    Phase 4 · Dynamic Validation
       11 sandbox replication                                      → C2.6
       12 dynamic exploitation (DAST)                              → C2.8
       13 exploit chaining                                         → C2.9
    Phase 5 · Governance & Reporting
       15 severity calibration, triaging and reporting             → C2.15
       14 remediation engineering, promoted to a merge request     → C2.16
...
top of a queue C2.15 has just shown to be sorted wrongly.

    Cross-cutting
       context engineering for the pipeline                        → C2.17
       injection in your own pipeline                              → B1.9
       securing the developers' coding agents                      → B3.11
       attesting control intent for agents and MCP servers         → C2.18

    Bonus
       Google Mantis — the pipeline in production                  → C2.19
"""
...
equivalent of a shape check, and C2.0 already established what those are worth.
""",
 "steps": [
  ("md", PIPELINE_NOTE),
...
discount, and you pay for them again in triage."""),
 ...
 "challenge": "Apply the path-slice rule where the source is three functions "
              ...
              "down entirely and the call graph the threat model derives (C2.2) "

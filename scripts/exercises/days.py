...
"B": {
 ...
 "day2": "The register in B1.18 is the scorecard. Count the risks that have a "
...
"C": {
 ...
 "day2": "This function is measured on a before-and-after you can state out "
         "loud. Deterministic SAST on the sample in C2.3 has precision 1.00 and "
...
"F": {
 ...
 "day2": "The unit is the key control indicator: computed from the estate, with "
         "a denominator, and a target written before the measurement. F1.13 "
...
"B1.1": ("Every later lesson names a component from this map. Without it a "
...
"B1.2": ("A user redirects their own agent past what the operator specified, "
...
"B1.3": ("Anyone who can write into a corpus the agent reads can steer it, "
...
"B1.4": ("The attacker's instruction outlives the conversation that delivered "
...
"B1.5": ("A legitimate tool with legitimate arguments does something nobody "
...
"B1.6": ("The agent acts with more authority than the person who asked, and "
...
"B1.7": ("You cannot say which agent acted, so you cannot revoke one without "
...
"B1.8": ("Model-authored code runs with the runtime's privileges, reaching the "
...
"B1.9": ("The pipeline reads attacker-controlled content and then acts on it — "
...
"B1.10": ("One compromised agent steers every agent downstream of it, because "
...
"B1.11": ("An agent nobody approved receives delegated work and delegated "
...
"B1.12": ("A single fabrication becomes a shared premise, and by the third hop "
...
"B1.13": ("The loop consumes budget, quota or downstream capacity without "
...
"B1.14": ("You cannot say which user caused an action, or what made the agent "
...
"B1.15": ("The approval gate is recorded as a control and operates as a click. "
...
"B1.16": ("The agent satisfies the letter of its instruction, including by "
...
"B1.17": ("The delegation chain becomes a privilege-laundering path, and the "
...
"B1.18": ("A list of risks is read once. Without a component, a control and an "
...
"B2.1": ("A shared service account answers \"what ran\" and destroys \"for "
...
"B2.2": ("A pre-shared secret in an image or an environment variable is "
...
"B2.3": ("Subset-only delegation lets a privileged user hand an agent "
...
"B2.4": ("Permanent scope makes every injection a successful one, because the "
...
"B2.5": ("Agents accumulate with no owner and no expiry, and an unregistered "
...
"B2.6": ("Concatenation destroys the one fact that separates an operator's "
...
"B2.7": ("Without the motivating input, root cause cannot be established at "
...
"B2.8": ("An agent that escapes its container can rewrite the record of what "
...
"B3.1": ("Allow-by-default authorization is defeated by any argument the model "
...
"B3.2": ("Model-authored code inherits the runtime's reach, including any "
...
"B3.3": ("An agent with unrestricted egress turns any successful injection "
         "into data loss.",
...
"B3.4": ("Without a ceiling the loop runs until an external system stops it, "
...
"B3.5": ("An unverified claim becomes a shared premise, and a peer's message "
...
"B3.6": ("An approval queue at volume approves everything, and the risk "
...
"B3.7": ("Per-agent controls diverge as the fleet grows, and legacy "
...
"B3.8": ("Any shared mutable surface is a channel between runs that are "
...
"B3.9": ("Classifiers off, no compensating cap, and tens of thousands of "
...
"B3.10": ("An agent that finds a live breach, reasons about telling somebody, "
...
"B3.11": ("The IDE agent holds git credentials, cloud credentials and a shell, "
...
"C2.0": ("A security pipeline built as if it were exempt from the risks it "
...
"C2.1": ("A harness whose verifier is the model agreeing with itself does not "
...
"C2.2": ("Threat models are written once, by hand, against a system that has "
         "changed since.",
...
"C2.3": ("Pattern matching floods the queue, and the classes that matter in "
...
"C2.4": ("Parallel analysis tracks report the same bug three times, and some "
...
"C2.5": ("A finding in dead code costs exactly as much to triage as one on the "
         "login path.",
...
"C2.6": ("Dynamic testing run against staging turns a destructive probe into "
         "an incident.",
...
"C2.7": ("A clean dependency scan on an estate carrying an undeclared "
...
"C2.8": ("A static finding is a hypothesis, and hypotheses get argued about in "
...
"C2.9": ("Three medium findings are triaged as three mediums, and nobody "
...
"C2.10": ("An offensive loop with no hard scope boundary tests something you "
...
"C2.11": ("Full source produces a finding list nobody can act on, because "
...
"C2.12": ("An agent narrates a confident architecture from status codes, and "
...
"C2.13": ("Object-level authorisation is assumed correct because the endpoint "
...
"C2.14": ("The offensive agent is the most capable and least supervised thing "
...
"C2.15": ("Severity copied from the rule orders the queue by something that "
          "predicts nothing.",
...
"C2.16": ("A patch that silences the scanner is indistinguishable from a patch "
          "that fixes the bug.",
...
"C2.17": ("The model is handed the repository and asked to be thorough, so the "
...
"C2.18": ("Control claims live in a spreadsheet and are never bound to a "
...
"C2.19": ("A reference implementation gets adopted as a product and its "
...
"D1.0": ("Offensive work that produces anecdotes — a result that worked once, "
...
"D1.1": ("The first surface an attacker reaches is the hub the model came "
...
"D1.2": ("A dataset is parsed before it is embedded, and a crafted record "
...
"D1.3": ("A jailbreak that strips safety while leaving tool use intact is the "
...
"D1.4": ("To catch the actor you have just played, you have to be able to see "
...
"D1.5": ("Each run examined alone is plausible work; the swarm exists only in "
...
"D1.6": ("A rule that flags an unauthorised runtime objective is worth a great "
...
"D1.7": ("An agent that scores evidence only on support confirms its first "
...
"D1.8": ("Every other detector needs a threshold, and every threshold trades "
...
"D1.9": ("A fleet completes thousands of actions inside one human approval "
...
"D1.10": ("A finding is only as good as its reproduction, and the constant "
...
"D1.11": ("A finding that stays in a notebook changes nothing and regresses "
          "unseen.",
...
"E1.0": ("A detection stack tuned for human tempo, watching an actor that acts "
...
"E1.1": ("Four products are bought, the estate is assumed covered, and the "
...
"E1.2": ("A detection that worked last month is silently degraded, because the "
...
"E1.3": ("An agent nobody registered acts under a person's credential, and "
...
"E2.1": ("Everything is indexed hot because nobody priced it, so retention is "
...
"E2.2": ("Classic baselines call two countries in an hour an incident and "
...
"E2.3": ("Platform-layer compromise is invisible to workload-layer detection — "
...
"E2.4": ("An agent writes candidate rules faster than anyone can review them, "
...
"E2.5": ("Every candidate rule catches the incident it was generated from, so "
...
"E2.6": ("Every other detector needs a threshold, and every threshold trades "
         "misses against false alarms.",
...
"E3.1": ("Supervising a loop by re-reading everything it did is not "
...
"E3.2": ("Incident response grants the broadest read in the organisation at "
...
"E3.3": ("An alert about an agent without scope, identity and delegation is "
...
"E3.4": ("Three responder instincts that are right for people misfire on "
...
"E3.5": ("A model correlates thousands of log lines in seconds and will "
...
"E3.6": ("An agent scoring evidence only on support confirms its first theory "
...
"E3.7": ("The agent that touched the resource is usually the last actor in a "
...
"E3.8": ("Coordination between runs that are supposed to be independent is "
...
"E3.9": ("Intelligence that stays a narrative about adversary trends moves "
...
"E3.10": ("Everything not covered by a rule is invisible, and the rules were "
...
"E4.1": ("Automation scope decided per runbook means the blast radius of your "
...
"E4.2": ("\"Automate everything\" and \"keep a human in it\" are both "
...
"E4.3": ("An agent at 300 actions a minute takes about 2,400 further actions "
...
"E4.4": ("Stop authority is the control everyone assumes exists and almost "
         "nobody has timed.",
...
"E4.5": ("Terminating agents while their credentials stay valid leaves the "
...
"E5.1": ("Miss one of the four runtime constants and you can describe what "
...
"E5.2": ("An incident that closes with a narrative recurs, because nothing in "
...
"E5.3": ("The fix goes to the prompt when the bug is in the control plane, and "
...
"E5.4": ("A closed ticket is not evidence that a control came back.",
...
"E5.5": ("Most incidents change what is deployed and leave what is allowed "
...
"E5.6": ("The clock starts at awareness, not at confirmation, and it does not "
...
"F1.0": ("A trustworthy-AI statement with no owner per property means every "
...
"F1.1": ("A control tested six months ago is not passing — it is unevidenced, "
...
"F1.2": ("You cannot govern, tier, test or revoke what you cannot list, and "
...
"F1.3": ("Tiering by model name tracks vendor marketing rather than exposure, "
...
"F1.4": ("A framework-first mapping produces a complete checklist that "
...
"F1.5": ("A vendor's best-of-k demo is not assurance, and schema conformance "
         "is not accuracy.",
...
"F1.6": ("Shipping only operating guardrails and reporting them as coverage is "
...
"F1.7": ("Point-in-time assurance describes a system that no longer exists by "
...
"F1.8": ("For a hosted model the estate holds no source to measure, so there "
...
"F1.9": ("A retrain, a fine-tune or a re-index is treated as maintenance, and "
...
"F1.10": ("Five functions each hold part of the AI control estate and none "
...
"F1.11": ("The classical model-risk playbook silently breaks once the model "
...
"F1.12": ("The functions work and the handoffs leak, and both sides are "
...
"F1.13": ("A control asserted in a register and never measured has no evidence "
...
"F2.1": ("One programme per regime is four times the work and none of it "
         "joined up.",
...
"F2.2": ("\"We only deployed it, we didn't build it\" is sometimes true and "
...
"F2.3": ("Regime-specific mappings have nothing to hang off, so each new law "
...
"F2.4": ("The sector regime you are already in applies to agents without ever "
         "using the word AI.",
...
"F2.5": ("The context window is a disclosure and the trace is a record, so "
...
"F2.6": ("The clock starts at awareness and broken attribution consumes it, "
...
"F2.7": ("Documentation that restates intent is what supervision fails.",
...
"F2.8": ("Without attribution and replay captured at the moment of the action, "
...
"F2.9": ("A supervisor who finds a weakness you did not disclose doubts "
...
"F3.1": ("A board cannot act on \"we found prompt injection\"; it can act on "
...
"F3.2": ("A per-tool review queue becomes a bottleneck and then a bypass, "
...
"F3.3": ("Starting with the most visible workflow rather than the most "
...
"F3.4": ("Harness engineering with no home and research as a hobby means the "
...
"F3.5": ("Reporting activity instead of exposure gives a board numbers that "
...
"F3.6": ("Saying no is cheap and usually wrong — the capability ships anyway "
...
"F3.7": ("Hiring for conceptual familiarity produces a team that can discuss "
...
"F3.8": ("A programme judged on prevention is judged on something a "
         "probabilistic system cannot deliver.",
         "Judge it on three capabilities instead: notice, stop, recover.",
         "Containment, detection and recovery indicators — the same ones E5.4 "

...
**Every lesson here runs a skill that a later function reuses.** G1.3 runs
`agent-identity-review` to *design* identity; A2.3 runs the same skill to
...
By the end of G2 you will have built every control the rest of the commons
refers to. Function A then tells you, component by component, exactly how each
one is bypassed.
""",
 "steps": [
  ("md", "## 2 · Draw it before you build it\n\n"
         "The skill below is the same one A1.1 uses to audit an architecture. "
 ...
 "challenge": "Add an eighth component: an egress gateway. CyberTravels does "
              "not have one, which is why A3.7 exists. Mark which trust "
...
saying "done" terminates on an opinion. B2.1 measures exactly this and finds a
...
The second and third are the ones that matter, and G1.7 builds them.
""",
 "steps": [
  ("md", "## 2 · The loop, and the verifier that is not the model\n\n"
         ...
         "The skill below is the one B2.1 uses to review a harness. Run it "
...
from a third party, they can change after you approved them, and A1.9 is the
...
""",
 "steps": [
  ("md", "## 2 · Two servers, and what each one declares\n\n"
         ...
         "says it can do, against what the code actually does. A1.9 runs it to "
...
authority for the action itself is minted separately — which is G1.4.
""",
 "steps": [
  ("md", "## 2 · Mint both, and read what makes them different\n\n"
         "`cybertravels/identity.py` mints these. The skill below is the one "
         "A2.3 uses to audit a delegation design; run it on yours while you "
...
""",
 "steps": [
  ("md", "## 2 · Exchange one, then watch a role refuse\n\n"
         ...
         "enforced rather than merely recorded. A2.6 runs it against a system "
...
a request, and A1.2 is the lesson on how far a request gets.
...
memory with no delete path makes that answer "no", and E2.4 is where that
...
""",
 "steps": [
  ("md", "## 2 · Write an untrusted sentence in, and watch it come back labelled\n\n"
         ...
         "scope and origin — A1.4 runs it to find a poisoning path; you are "
...
""",
 "steps": [
  ("md", "## 2 · Forge a peer message, and watch verification refuse it\n\n"
         ...
         "A1.7 runs it to follow an injection; you are running it to see what "
...
per hour rather than by whether it exists. A3.9 is the lesson that models it.
...
""",
 "steps": [
  ("md", "## 2 · Approve one, refuse one, then exhaust the budget\n\n"
         ...
         "what a run returns when one does. A1.13 runs it on somebody else's "
...
- **Refusals are spans too.** A trace holding only successful calls hides
  exactly the events D2 will want to write rules against.
""",
 "steps": [
...
convention, and A2.8 is the lesson on making it a property: separate
...
""",
 "steps": [
  ("md", "## 2 · Put the four questions to your own rows\n\n"
         ...
         "which it cannot answer. A1.14 runs it on an incident; you are running "
 ...
 "challenge": "Add the motivating input and its origin to the audit row. Then "
              ...
              "traveller text into a long-lived store, which is E2.5's problem.",
},
...
- **The suite can be diluted.** Add thirty easy cases everything passes and the
  ...
  most common way an eval number improves, and C1.8 measures it directly.
...
""",
 "steps": [
  ("md", "## 2 · Score it, then dilute the suite and score it again\n\n"
         ...
         "cases everything passes, quietly lifting the number. C1.8 uses it on "
...
|---|---|
| ingress accepting traveller text | A1.2 — prompt injection |
| the vendor MCP server | A1.3 — indirect injection, through a document you fetched |
| tool descriptions you trusted | A1.9 — a rug-pull after approval |
| memory with origin recorded | A1.4 — what happens when the origin is dropped |
| agent-to-agent envelopes | A1.7 — one injection reaching four agents |
| the per-action token exchange | A2.3 — and the systems that log it without enforcing it |
| the human gate | A3.9 — at four hundred approvals a day |
| the audit trail | A1.14 — and the question it still cannot answer |

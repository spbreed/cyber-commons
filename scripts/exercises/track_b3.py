"""B3 — Controls: runtime, and the gateway.

Chapter B2 answered "who is calling" and "where did this text come from". This
...
of these controls once you run more than a handful of agents.
    ...
    B3.1  default-deny on the tool call   T2, T3, T6
    B3.2  sandboxed execution             T11, T2
    B3.3  egress control                  T2, T6, LLM02
    B3.4  budgets and stop conditions     T4, T10
    B3.5  validating what comes back      T5, T12, T7
    B3.6  approval that survives volume   T10, T15
    B3.7  the gateway                     all of the above, once
"""
...
""",
 "steps": [
  ("md", MITIGATES + "> Changes what the executing process can **reach**, which "
         "is the only variable B1.8 turned on. A sandbox holding production "

"""B1 (part 1) — The AppSec pipeline, phases 1 to 3. Sessions C2.1–C2.5.
...
order the stages do.
    ...
    Before any stage: what a harness is                        → C2.1
    ...
    Phase 2 · Threat Modelling
        5 threat modelling, from six static inputs                 → C2.2
    Phase 3 · Analysis & Filtering
        7 vulnerability auditing: real Semgrep, then the model pass → C2.3
        8 deduplication             9 contextual verification      → C2.4
       10 feasibility filtering                                    → C2.5
...
none, so it is taught where the money actually goes: the audit agent in C2.3
decides where to run the model pass, and C2.5 decides what is worth
reproducing.

Phases 4 and 5 continue in track_b1b.py, and C2.19 closes the track with Google
...
("md", "## 4 · Stage 9 — contextual verification against the real syntax\n\n"
 ...
 "expect": "Eight raw findings collapse to five distinct defects, with the "
           ...
           "`search_bookings`, C2.3's IDOR hypothesis on `get_booking`, and one "
...
("md", "## 4 · The control — route each bucket to a different place"),
  ...
  *skill_steps("appsec/dead-code-ast-reachability",
               ...
               "The whole `cybertravels/` tree \u2014 the same repository C2.3 "

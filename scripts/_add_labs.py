...
NEW = {
 # ---------------- B1 Security Architect ----------------
 "B1.1": {"goal": "Make the threat model a living artefact that moves when the tools move.",
 ...
 "B1.4": {"goal": "Put a blast-radius number in the design review.",
  "run": ["cd labs/a1-control-plane",
 ...
 "B1.5": {"goal": "Find where authority actually concentrates in a multi-agent topology.",
  "run": ["cd labs/a1-control-plane",
 ...
 "B1.6": {"goal": "Score gateways against a fixed control checklist instead of a feature matrix.",
  "run": ["cd labs/a1-control-plane",
 ...
 "B1.7": {"goal": "Prove the router fails closed, not open.",
 ...
 # ---------------- B2 Identity / NHI ----------------
 "B2.1": {"goal": "Answer 'who is calling?' for every principal in the lab.",
  "run": ["cd labs/b2-delegation",
 ...
 "B2.3": {"goal": "Find agents running on inherited human credentials.",
  "run": ["cd labs/b2-delegation",
 ...
 "B2.4": {"goal": "Revoke exactly one agent without collateral.",
  "run": ["cd labs/b2-delegation",
 ...
 "B2.6": {"goal": "Move every credential out of the agent and into the gateway.",
  "run": ["cd labs/b2-delegation",
 ...
 "B2.7": {"goal": "Teach a legacy service to refuse agent writes it cannot understand.",
  "run": ["cd labs/b2-delegation",
 ...
 "B2.9": {"goal": "Reproduce the four classic identity failures, then close them.",
  "run": ["cd labs/b2-delegation/classic-failures",
 ...
 # ---------------- B3 Platform ----------------
 "B3.3": {"goal": "Stop the agent wandering from code into credentials.",
 ...
 "B3.4": {"goal": "Escalate through connector chaining, then pin and scan to stop it.",
 ...
 "B3.5": {"goal": "Design the dangerous call out of existence.",
 ...
 "B3.7": {"goal": "Detect an unmanaged personal agent on a managed endpoint.",
  "run": ["cd labs/a3-sandbox",
 ...
 "B3.8": {"goal": "Make dev-agent credentials structurally unable to reach production.",
  "run": ["cd labs/a3-sandbox",
 ...
 # ---------------- B1 AppSec ----------------
 "C2.2": {"goal": "Beat context stuffing with tool-mediated retrieval.",
 ...
 "C2.4": {"goal": "Chart honestly what only SAST found, and what only DAST found.",
  "run": ["cd labs/b1-appsec",
 ...
 "C2.4": {"goal": "Find live secrets in your own agent's footprint.",
 ...
 "C2.5": {"goal": "Publish the four numbers that let a review workflow earn its next rung.",
  "run": ["cd labs/b1-appsec",
 ...
 # ---------------- C2 Harness ----------------
 "E2.6": {"goal": "Refactor a shell tool into three narrow, structured tools.",
 ...
 "C2.5": {"goal": "Route cheap executor to escalated reasoner and attribute spend.",
 ...
 "C2.6": {"goal": "Prove a grandchild agent cannot exceed its parent.",
  "run": ["cd labs/b2-delegation",
 ...
 "C2.7": {"goal": "Name the failure from the trace alone.",
 ...
 "C2.8": {"goal": "Let a scaffold mutate itself, and keep only measured gains.",
  "run": ["cd labs/m0-agent-loop",
 ...
 "C2.9": {"goal": "Replay an agent run from its trace.",
  "run": ["cd labs/m0-agent-loop",
 ...
 # ---------------- D1 Red team ----------------
 "D1.2": {"goal": "Prove the offensive harness cannot reach anything you don't own.",
 ...
 "D1.4": {"goal": "Attack the delegation chain and see if attenuation holds.",
  "run": ["cd labs/c1-redteam",
          "python3 attack_identity.py --target ../b2-delegation --technique confused-deputy",
          "python3 attack_identity.py --target ../b2-delegation --technique token-replay",
          "python3 attack_identity.py --target ../b2-delegation --technique scope-escalation",
          "python3 attack_identity.py --verify-revocation"],
  "expect": "A pass/fail per technique against B2's chain — including whether revocation actually revokes."},
 "D1.5": {"goal": "Attack the sandbox from inside and measure what leaves.",
  "run": ["cd labs/c1-redteam",
          "python3 escape.py --target ../a3-sandbox --techniques mount,egress,path-guard,mcp",
          "python3 escape.py --measure-exfil --bytes-out"],
  "expect": "Blast radius in bytes and reachable hosts. B3's deliverable is zero outside the sandbox."},
 "D1.7": {"goal": "Write a finding for an emergent behaviour, not a line of code.",
  "run": ["cd labs/c1-redteam",
 ...
 "C2.8": {"goal": "Turn a finding into a policy another track deploys.",
  ...
  "expect": "A tested Rego policy handed to B3 — research output the platform team can actually deploy."},
 ...
 # ---------------- E1 SOC ----------------
 "E3.3": {"goal": "Show a context-loaded triage loop beating a generic one.",
  "run": ["cd labs/d1-soc",
 ...
 "E2.4": {"goal": "Generate, unit-test and tune detections inside CI.",
 ...
 "E1.3": {"goal": "Get agent telemetry into the SIEM and query it.",
  "run": ["cd labs/d1-soc",
 ...
 "E1.2": {"goal": "Catch a detection silently degrading after a model change.",
  "run": ["cd labs/d1-soc",
 ...
 "E3.9": {"goal": "Build a synthesis loop that must cite or abstain.",
 ...
 # ---------------- E2 IR ----------------
 "E3.5": {"goal": "Pre-load the agent so it reasons as a partner, not a tool you reach for late.",
  "run": ["cd labs/d2-ir",
 ...
 "E3.7": {"goal": "Scope an incident where the initiating agent is not the acting one.",
  "run": ["cd labs/d2-ir",
 ...
 "E5.3": {"goal": "Pick the right layer to change after an incident.",
 ...
 "E4.4": {"goal": "Time your stop authority end to end.",
  "run": ["cd labs/d2-ir",
 ...
 "E5.6": {"goal": "Run the first-hour regulatory checklist.",
  "run": ["cd labs/d2-ir",
 ...
 # ---------------- F1 GRC ----------------
 "F1.1": {"goal": "Watch control evidence go stale without a code change.",
  "run": ["cd labs/e1-grc",
 ...
 "F1.3": {"goal": "Tier by what the thing can do, not by model name.",
  "run": ["cd labs/e1-grc",
 ...
 "F1.4": {"goal": "Map agent controls onto the library you already have.",
  "run": ["cd labs/e1-grc",
 ...
 "F1.6": {"goal": "Sort your guardrails into operating vs outcome.",
  "run": ["cd labs/e1-grc",
 ...
 "F1.9": {"goal": "Write the gate a re-index has to pass.",
  "run": ["cd labs/e1-grc",
 ...
 # ---------------- F2 Regulatory ----------------
 "F2.1": {"goal": "Build one control set that answers several regimes.",
 ...
 "F2.2": {"goal": "Classify three workflows and defend the boundary cases.",
  "run": ["cd labs/e2-compliance",
 ...
 "F2.3": {"goal": "Hang two regulator mappings off one framework spine.",
  "run": ["cd labs/e2-compliance",
 ...
 "F2.4": {"goal": "Find the regime you are already in.",
  "run": ["cd labs/e2-compliance",
 ...
 "F2.6": {"goal": "Draft the notification for an autonomous actor.",
  "run": ["cd labs/e2-compliance",
 ...
 "F2.7": {"goal": "Assemble a pack that survives supervision.",
  "run": ["cd labs/e2-compliance",
 ...
 "F2.9": {"goal": "Defend one workflow in a mock supervisory conversation.",
  "run": ["cd labs/e2-compliance",
 ...
 # ---------------- F3 CISO office ----------------
 "F3.1": {"goal": "Turn a blast-radius measurement into a board paragraph.",
  "run": ["cd labs/e3-ciso",
 ...
 "F3.2": {"goal": "Replace a per-tool review queue with a delegated-authority policy.",
 ...
 "F3.3": {"goal": "Sequence the first three workflows — and name the first hard no.",
  "run": ["cd labs/e3-ciso",
 ...
 "F3.4": {"goal": "Draw your org's ownership map against the topic matrix.",
  "run": ["cd labs/e3-ciso",
 ...
 "F3.6": {"goal": "Make a conditional approval enforceable rather than aspirational.",
  "run": ["cd labs/e3-ciso",
 ...
 "F3.7": {"goal": "Write an interview loop that separates familiarity from practice.",
  "run": ["cd labs/e3-ciso",
 ...
 "F3.8": {"goal": "Re-score your programme on containment/detection/recovery.",
  "run": ["cd labs/e3-ciso",

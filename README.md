...
**127 lessons across 12 chapters.** Every lesson is a self-contained Python
...
1. Open [**A1.1**](https://spbreed.github.io/cyber-commons/lessons/A1.1.html).
...
Locally, if you prefer: `python3 scripts/run_notebooks.py --session A1.1`
...
> **concept → demo → where it breaks → control → verify**

The idea is introduced and *demonstrated working* before any risk is raised —
leading with the threat teaches people to fear a mechanism they cannot describe.
Then the failure is reproduced in code from the mechanism you just saw. Then the
control, and a check that it holds. The build **fails** on a lesson without a
`concept` section.
...
> **Spine:** A1.1 → A1.2 → A1.5 → **A1.9** → **A1.10** → **A1.11** → A2.1 →
> A2.3 → A2.4 → A3.2 → A3.5 → A3.6 → **B2.0** → B2.1 → B2.2 → D1.5 →
> **E1.0** → E1.2 → **E1.10** → E3.2

| Function | Chapters | Lessons |
|---|---|---|
| **A · Security Architecture & Platform** | [1](curriculum/track-a1.md) Security Architect · [2](curriculum/track-a2.md) Identity & NHI · [3](curriculum/track-a3.md) Platform & Cloud | 30 |
| **B · Product & Application Security** | [4](curriculum/track-b1.md) AppSec / Code Review · [5](curriculum/track-b2.md) Security Automation & Harness | 34 |
| **C · Offensive Security & Research** | [6](curriculum/track-c1.md) Pentest / Red Team · [7](curriculum/track-c2.md) Research | 16 |
| **D · Security Operations** | [8](curriculum/track-d1.md) SOC & Detection · [9](curriculum/track-d2.md) Incident Response | 16 |
| **E · Governance, Risk & the CISO Office** | [10](curriculum/track-e1.md) GRC · [11](curriculum/track-e2.md) Regulatory · [12](curriculum/track-e3.md) BISO / CISO Office | 30 |
| | **12 chapters** | **127** |

Two chapters carry a single artefact end to end:

- **[A1](curriculum/track-a1.md)** opens with a vendor-agnostic reference
  architecture for agentic AI, then works through the risks one per lesson
  (grounded in the OWASP Agentic Top 10), then the controls that answer them —
  agent identity, default-deny, sandboxing, ingress, gateway.
...
instrument) and **Security for AI** (agents as the thing you defend). Teaching
...
one is caught in nine seconds instead of after 127 remote pushes.
...
CI re-runs all of it with `--check` — secret scan, 127 notebooks, determinism
...
```
site/data/curriculum.json   source of truth: 127 sessions, 12 chapters
...
labs/notebooks/             127 generated notebooks + execution and Kaggle evidence

# Lab B2.5 — delegation that survives audit

**Chapter:** [Track B2 — Identity & NHI Engineer](../../curriculum/track-b2.md) ·
also backs B2.3 (Shadow Autonomy), B2.4 (revocation), C2.0 (delegation depth),
D1.4 (attacking the chain)
...
[track-b2.md](../../curriculum/track-b2.md); this file is that lesson with the
...
**That nested `act` claim is the audit trail** ([F2.8](../../curriculum/track-f2.md)).
...
| `impersonate` | Hand the agent alice's token unchanged and nothing looks broken — the audit log just says "alice deployed to prod". That's **Shadow Autonomy** (B2.3), and it survives in production for years precisely because it isn't an error |
| `revoke` | One actor dies, the other keeps working — the B2.4 deliverable ("revoke one misbehaving agent without breaking forty others") |
...
([B1.3](../../curriculum/track-b1.md)): the over-privileged grant isn't blocked
...
- Chain to depth 5 and ask where you would cap it — that's
  [C2.0](../../curriculum/track-c2.md), delegation-depth budgets.

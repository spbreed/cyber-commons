#!/usr/bin/env python3
"""Lab B2.5 — a three-hop delegation chain that survives audit.
...
def cmd_impersonate() -> int:
    ...
    print("\nThis is Shadow Autonomy (B2.3). Nothing here is technically broken —\n"
...
def cmd_revoke(target: str) -> int:
    ...
    for name, tok in (("reviewer-agent chain", t1), ("patch-agent chain", t_other)):
        try:
            ...
            print(f"  {name}: dead — {e}")
    print("\nOne actor revoked, the other unaffected: the B2.4 deliverable.")
    return 0

...
per converted lesson (`a0-0-set-up-your-computer`), generated by
...
lessons each, so an audit cannot be named for one lesson. Which lessons are
converted is `exercises/lessonskills.py`; the rest still run their audit
directly. Link them with `--lessons`:
...
| 20 | `build_lesson_skills.py --check` | a lesson skill stale against the lesson it teaches, a `lesson-skills/` directory no converted lesson owns (it would still load and still answer), a skill whose description does not open with its lesson id, or one naming an audit that is not in `skills/` |

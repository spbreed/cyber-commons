...
runs it; there is never a second copy of the procedure to correct. Every
...
happened, and calls the audit skill rather than repeating it (§3).
...
| `scripts/exercises/lessonskills.py` — a lesson's skill name, and which audits it runs | the run block on a page (`build_site.lesson_skill_block`) and in a chapter doc (`build_curriculum.lesson_run`); it is derived, not typed |
...
| | `LIGHTBOARD.md` |
| | `lesson-skills/*` — the skill a learner picks to do a lesson, from `build_lesson_skills.py` |
...
| a lesson's skill name | `scripts/exercises/lessonskills.py` |
...
**A lesson is also a skill you pick.** `lesson-skills/<name>/` holds one skill
per lesson (`a0-0-set-up-your-computer`), generated by
`build_lesson_skills.py`. It teaches the idea, runs `scripts/lesson.py` — which
gets that lesson's CyberTravels checkpoint, runs its tests and its audit, and
prints a four-heading readback — and reads the result back. It calls the audit
skill under `skills/` and never copies it: eighteen audits serve two or three
lessons each, so an audit cannot be named for one lesson. Link them with
`--lessons`:

```bash
python3 scripts/install_skills.py --all --lessons A0.0   # one lesson (or A, A1, all)
python3 scripts/install_skills.py --tool copilot --lessons A   # .github/skills in this repo
python3 scripts/install_skills.py --tool agents --lessons A    # .agents/skills: Codex, Cursor, Copilot
```

On Windows a failed symlink falls back to a directory junction, which needs no
Developer Mode and is still a link, not a copy.
...
| 20 | `build_lesson_skills.py --check` | a lesson skill stale against the lesson it teaches, a `lesson-skills/` directory no lesson owns (it would still load and still answer), a skill whose description does not open with its lesson id, or one naming an audit that is not in `skills/` |
| 21 | `check_frameworks.py` | a framework label that is not real |
| 22 | `check_framework_links.py` | a framework label that links nowhere. `continue-on-error`: an upstream site being down must not hold the deploy |
| 23 | `build_site.py` | a page stale against its source. It rebuilds and warns rather than failing, so a forgotten rebuild never blocks a deploy |
| 24 | curriculum + videos data | `videos.json` naming a session id the curriculum does not carry |
...
- **The skills stores are `skills/` and `lesson-skills/`, with links out of
  them.** Never copy a skill into a tool's directory to "make it work" — a copy
  is a fork, and the next fix reaches one of them. A symlink and a Windows
  junction are both links; a copied folder is not.
...
- **Measure "runs a skill" from the lesson sources, never from a generated
  ...
  `EXERCISES` once held orphans from the Function D trim (C2.8–C2.10) that were
  not lessons, and counting them made the "132 of 135" claim silently false.

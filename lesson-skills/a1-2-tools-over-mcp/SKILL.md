---
name: a1-2-tools-over-mcp
description: >-
  A1.2 — Tools over MCP. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of A1.2, runs it, and reports what happened. Use when working through lesson A1.2 or when asked to do A1.2.
license: Apache-2.0
...
The learner wants to do lesson **A1.2**: they typed `/a1-2-tools-over-mcp`, picked it from
their agent's skill list, or asked to "do A1.2". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py A1.2`. It gets the CyberTravels code as it stood at
   the end of A1.2 into `work/cybertravels`, shows what this lesson added,
...
3. **The audit.**
   - The audit, `skills/attestation/agent-code-surface-analyzer`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py A1.2 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Change one tool's description to claim it is read-only while leaving it a write. Nothing in the protocol stops you. That is B1.9, from the inside.

---
name: f3-4-org-design-and-ownership
description: >-
  F3.4 — Org design and ownership. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F3.4, runs it, and reports what happened. Use when working through lesson F3.4 or when asked to do F3.4.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F3.4
  commons-audit: programme/ownership-seam-audit
...
The learner wants to do lesson **F3.4**: they typed `/f3-4-org-design-and-ownership`, picked it from
their agent's skill list, or asked to "do F3.4". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F3.4`. It gets the CyberTravels code as it stood at
   the end of F3.4 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/programme/ownership-seam-audit`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/programme/ownership-seam-audit`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F3.4 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Put a name against each of the six seams this week. Any seam where two people both say "them" is the one that will stall your next incident, and finding it now costs one meeting.

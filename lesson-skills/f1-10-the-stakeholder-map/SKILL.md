---
name: f1-10-the-stakeholder-map
description: >-
  F1.10 — The stakeholder map. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F1.10, runs it, and reports what happened. Use when working through lesson F1.10 or when asked to do F1.10.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F1.10
  commons-audit: programme/stakeholder-seam-map
...
The learner wants to do lesson **F1.10**: they typed `/f1-10-the-stakeholder-map`, picked it from
their agent's skill list, or asked to "do F1.10". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F1.10`. It gets the CyberTravels code as it stood at
   the end of F1.10 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/programme/stakeholder-seam-map`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/programme/stakeholder-seam-map`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F1.10 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Pick one of the four seams and find out, today, who owns it in your organisation. The answer 'I assume security does' from one side and 'I assume privacy does' from the other is the finding.

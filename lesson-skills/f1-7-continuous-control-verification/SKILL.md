---
name: f1-7-continuous-control-verification
description: >-
  F1.7 — Continuous control verification. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F1.7, runs it, and reports what happened. Use when working through lesson F1.7 or when asked to do F1.7.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F1.7
  commons-audit: attestation/aws-runtime-posture-collector
...
The learner wants to do lesson **F1.7**: they typed `/f1-7-continuous-control-verification`, picked it from
their agent's skill list, or asked to "do F1.7". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F1.7`. It gets the CyberTravels code as it stood at
   the end of F1.7 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/attestation/aws-runtime-posture-collector`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/attestation/aws-runtime-posture-collector`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F1.7 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Automate the control with the shortest freshness window first — it is the one costing the most manual effort and going stale most often. One automated test converts an annual assertion into a live control.

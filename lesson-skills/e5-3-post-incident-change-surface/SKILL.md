---
name: e5-3-post-incident-change-surface
description: >-
  E5.3 — Post-incident change surface. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E5.3, runs it, and reports what happened. Use when working through lesson E5.3 or when asked to do E5.3.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E5.3
  commons-audit: response/post-incident-change-surface
...
The learner wants to do lesson **E5.3**: they typed `/e5-3-post-incident-change-surface`, picked it from
their agent's skill list, or asked to "do E5.3". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E5.3`. It gets the CyberTravels code as it stood at
   the end of E5.3 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/response/post-incident-change-surface`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/response/post-incident-change-surface`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E5.3 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Take your last incident's action list and mark each item's landing surface. Anything landing in a console has no record and no verification path — move those into git before the next one.

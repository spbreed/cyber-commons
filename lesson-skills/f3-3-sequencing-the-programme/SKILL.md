---
name: f3-3-sequencing-the-programme
description: >-
  F3.3 — Sequencing the programme. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F3.3, runs it, and reports what happened. Use when working through lesson F3.3 or when asked to do F3.3.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F3.3
  commons-audit: programme/programme-sequencing
...
The learner wants to do lesson **F3.3**: they typed `/f3-3-sequencing-the-programme`, picked it from
their agent's skill list, or asked to "do F3.3". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F3.3`. It gets the CyberTravels code as it stood at
   the end of F3.3 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/programme/programme-sequencing`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/programme/programme-sequencing`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F3.3 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Locate your programme on the six steps honestly. Most are between 2 and 3 while reporting on 5, which is exactly the gap this sequence prevents — and the fix is to stop reporting 5 until 2 and 3 are done.

---
name: e3-10-hunting-in-agent-telemetry
description: >-
  E3.10 — Hunting in agent telemetry. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E3.10, runs it, and reports what happened. Use when working through lesson E3.10 or when asked to do E3.10.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E3.10
  commons-audit: detection/agent-telemetry-hunt
...
The learner wants to do lesson **E3.10**: they typed `/e3-10-hunting-in-agent-telemetry`, picked it from
their agent's skill list, or asked to "do E3.10". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E3.10`. It gets the CyberTravels code as it stood at
   the end of E3.10 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/detection/agent-telemetry-hunt`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/detection/agent-telemetry-hunt`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E3.10 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Write a fourth hypothesis for CyberTravels and score it. If its precision is near the base rate, you have described normal work.

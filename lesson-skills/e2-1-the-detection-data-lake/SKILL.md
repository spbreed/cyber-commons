---
name: e2-1-the-detection-data-lake
description: >-
  E2.1 — The detection data lake. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E2.1, runs it, and reports what happened. Use when working through lesson E2.1 or when asked to do E2.1.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E2.1
  commons-audit: detection/telemetry-tiering-cost
...
The learner wants to do lesson **E2.1**: they typed `/e2-1-the-detection-data-lake`, picked it from
their agent's skill list, or asked to "do E2.1". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E2.1`. It gets the CyberTravels code as it stood at
   the end of E2.1 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/detection/telemetry-tiering-cost`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/detection/telemetry-tiering-cost`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E2.1 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** List the five queries your SOC actually ran last month, then tier your sources from them. Any source that appears in no query is the finding — you are paying to store something nobody asks.

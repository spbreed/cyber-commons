---
name: e1-1-the-sensor-estate
description: >-
  E1.1 — The sensor estate. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E1.1, runs it, and reports what happened. Use when working through lesson E1.1 or when asked to do E1.1.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E1.1
  commons-audit: detection/sensor-coverage-matrix
...
The learner wants to do lesson **E1.1**: they typed `/e1-1-the-sensor-estate`, picked it from
their agent's skill list, or asked to "do E1.1". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E1.1`. It gets the CyberTravels code as it stood at
   the end of E1.1 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/detection/sensor-coverage-matrix`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/detection/sensor-coverage-matrix`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E1.1 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Build the same matrix for your estate with your own actions in the rows. The number that matters is not the percentage — it is whether the uncovered rows have something in common.

---
name: b3-1-default-deny-on-the-tool-call
description: >-
  B3.1 — Default-deny on the tool call. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B3.1, runs it, and reports what happened. Use when working through lesson B3.1 or when asked to do B3.1.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B3.1
  commons-audit: attestation/iam-least-privilege-verifier
...
The learner wants to do lesson **B3.1**: they typed `/b3-1-default-deny-on-the-tool-call`, picked it from
their agent's skill list, or asked to "do B3.1". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B3.1`. It gets the CyberTravels code as it stood at
   the end of B3.1 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/attestation/iam-least-privilege-verifier`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/attestation/iam-least-privilege-verifier`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B3.1 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Take one tool policy you have and check whether it names the resource *and* the verb. If it grants `run_query` rather than `SELECT on these tables`, it cannot express the difference that B1.5 and R1 both turn on.

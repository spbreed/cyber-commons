---
name: c2-1-what-building-a-harness-means-in
description: >-
  C2.1 — What building a harness means in security engineering. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of C2.1, runs it, and reports what happened. Use when working through lesson C2.1 or when asked to do C2.1.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: C2.1
  commons-audit: appsec/agentic-harness-loop
...
The learner wants to do lesson **C2.1**: they typed `/c2-1-what-building-a-harness-means-in`, picked it from
their agent's skill list, or asked to "do C2.1". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py C2.1`. It gets the CyberTravels code as it stood at
   the end of C2.1 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/appsec/agentic-harness-loop`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/appsec/agentic-harness-loop`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py C2.1 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Name your pipeline's verifier out loud. If the sentence contains "the model checks" or "it looks right", you have a judge, and a judge approves confident prose — including prose that contradicts the finding it is attached to.

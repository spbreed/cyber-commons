...
name: c2-14-bonus
description: >-
  C2.14 — Bonus. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of C2.14, runs it, and reports what happened. Use when working through lesson C2.14 or when asked to do C2.14.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: C2.14
  commons-audit: redteam/offensive-agent-safety-preflight
...
The learner wants to do lesson **C2.14**: they typed `/c2-14-bonus`, picked it from
their agent's skill list, or asked to "do C2.14". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py C2.14`. It gets the CyberTravels code as it stood at
   the end of C2.14 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/redteam/offensive-agent-safety-preflight`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/redteam/offensive-agent-safety-preflight`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py C2.14 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Write your own engagement scope as a structure the harness reads, not a paragraph in a PDF, and run this preflight against it. The control you cannot currently satisfy is the one to fix before the next engagement, not after it.

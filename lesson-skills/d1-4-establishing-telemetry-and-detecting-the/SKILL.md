---
name: d1-4-establishing-telemetry-and-detecting-the
description: >-
  D1.4 — Establishing telemetry and detecting the actor. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of D1.4, runs it, and reports what happened. Use when working through lesson D1.4 or when asked to do D1.4.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: D1.4
  commons-audit: detection/agent-versus-human-scoring
...
The learner wants to do lesson **D1.4**: they typed `/d1-4-establishing-telemetry-and-detecting-the`, picked it from
their agent's skill list, or asked to "do D1.4". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py D1.4`. It gets the CyberTravels code as it stood at
   the end of D1.4 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/detection/agent-versus-human-scoring`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/detection/agent-versus-human-scoring`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py D1.4 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Check whether your model gateway logs the acting identity per call. If it logs only the API key, every agent is anonymous and this scoring is the only actor you have.

---
name: f3-1-translating-agentic-risk-upward
description: >-
  F3.1 — Translating agentic risk upward. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F3.1, runs it, and reports what happened. Use when working through lesson F3.1 or when asked to do F3.1.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F3.1
  commons-audit: programme/risk-translation-upward
...
The learner wants to do lesson **F3.1**: they typed `/f3-1-translating-agentic-risk-upward`, picked it from
their agent's skill list, or asked to "do F3.1". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F3.1`. It gets the CyberTravels code as it stood at
   the end of F3.1 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/programme/risk-translation-upward`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/programme/risk-translation-upward`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F3.1 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Write these four lines for your highest-tier system. If you cannot fill the likelihood line with a measurement, that is the first thing to fund — an assessment is not a number.

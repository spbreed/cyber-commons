---
name: f3-8-resilience-over-perfection
description: >-
  F3.8 — Resilience over perfection. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F3.8, runs it, and reports what happened. Use when working through lesson F3.8 or when asked to do F3.8.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F3.8
  commons-audit: programme/resilience-readiness-check, research/research-durability-check
...
The learner wants to do lesson **F3.8**: they typed `/f3-8-resilience-over-perfection`, picked it from
their agent's skill list, or asked to "do F3.8". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F3.8`. It gets the CyberTravels code as it stood at
   the end of F3.8 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/programme/resilience-readiness-check`, `skills/research/research-durability-check`). It ends with a READBACK.
3. **The audit.**
   - Audit 1 of 2, `skills/programme/resilience-readiness-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F3.8 --audit 1 --answer work/answer.json`.
   - Audit 2 of 2, `skills/research/research-durability-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F3.8 --audit 2 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Run a game day that assumes containment failed. Measure notice, stop and recover as three separate numbers. The weakest one is next quarter's plan — and unlike a prevention target, you can actually reach it.

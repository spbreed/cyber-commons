---
name: b1-4-memory-poisoning
description: >-
  B1.4 — Memory poisoning. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B1.4, runs it, and reports what happened. Use when working through lesson B1.4 or when asked to do B1.4.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B1.4
  commons-audit: threats/memory-scope-and-origin-audit
...
The learner wants to do lesson **B1.4**: they typed `/b1-4-memory-poisoning`, picked it from
their agent's skill list, or asked to "do B1.4". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B1.4`. It gets the CyberTravels code as it stood at
   the end of B1.4 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/threats/memory-scope-and-origin-audit`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/threats/memory-scope-and-origin-audit`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B1.4 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Look at what your agent writes to long-term memory and ask which of it originated in content a user did not author. Then ask what would remove it, and who would notice it was there.

---
name: e4-5-the-fleet-kill-switch
description: >-
  E4.5 — The fleet kill switch. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E4.5, runs it, and reports what happened. Use when working through lesson E4.5 or when asked to do E4.5.
license: Apache-2.0
...
The learner wants to do lesson **E4.5**: they typed `/e4-5-the-fleet-kill-switch`, picked it from
their agent's skill list, or asked to "do E4.5". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E4.5`. It gets the CyberTravels code as it stood at
   the end of E4.5 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/response/fleet-kill-switch-test`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/response/fleet-kill-switch-test`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E4.5 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Ask who in your organisation is allowed to stop every agent at once, without asking anyone. If the answer is a committee, you do not have a kill switch — you have an escalation path, and they take different amounts of time.

---
name: b1-3-indirect-prompt-injection
description: >-
  B1.3 — Indirect prompt injection. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B1.3, runs it, and reports what happened. Use when working through lesson B1.3 or when asked to do B1.3.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B1.3
  commons-audit: threats/indirect-injection-path-trace
...
The learner wants to do lesson **B1.3**: they typed `/b1-3-indirect-prompt-injection`, picked it from
their agent's skill list, or asked to "do B1.3". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B1.3`. It gets the CyberTravels code as it stood at
   the end of B1.3 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/threats/indirect-injection-path-trace`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/threats/indirect-injection-path-trace`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B1.3 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** List the trust-0 and trust-1 components in one agent you operate and name who can write into each. Most teams find a path they had not counted, and it is usually a tool result: the output of a system they trust, carrying text a stranger wrote.

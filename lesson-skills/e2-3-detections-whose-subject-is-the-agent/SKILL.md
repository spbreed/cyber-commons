---
name: e2-3-detections-whose-subject-is-the-agent
description: >-
  E2.3 — Detections whose subject is the agent platform. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E2.3, runs it, and reports what happened. Use when working through lesson E2.3 or when asked to do E2.3.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E2.3
  commons-audit: detection/agent-platform-detections
...
The learner wants to do lesson **E2.3**: they typed `/e2-3-detections-whose-subject-is-the-agent`, picked it from
their agent's skill list, or asked to "do E2.3". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E2.3`. It gets the CyberTravels code as it stood at
   the end of E2.3 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/detection/agent-platform-detections`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/detection/agent-platform-detections`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E2.3 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Pick the orphaned-process rule and write it for your own platform. It is one query, it has a low false-positive rate, and on most agent platforms nobody has ever run it.

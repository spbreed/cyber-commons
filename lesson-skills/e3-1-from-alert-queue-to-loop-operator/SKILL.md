---
name: e3-1-from-alert-queue-to-loop-operator
description: >-
  E3.1 — From alert queue to loop operator. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E3.1, runs it, and reports what happened. Use when working through lesson E3.1 or when asked to do E3.1.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E3.1
  commons-audit: detection/triage-loop-with-floor
...
The learner wants to do lesson **E3.1**: they typed `/e3-1-from-alert-queue-to-loop-operator`, picked it from
their agent's skill list, or asked to "do E3.1". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E3.1`. It gets the CyberTravels code as it stood at
   the end of E3.1 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/detection/triage-loop-with-floor`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/detection/triage-loop-with-floor`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E3.1 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Ask your SOC one question: when an incident is confirmed, does anyone check whether an earlier alert about it was closed? If nobody does, you have no measurement of your false-negative rate — with or without an agent.

...
name: f3-6-saying-no
description: >-
  F3.6 — Saying no, and saying yes with conditions. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F3.6, runs it, and reports what happened. Use when working through lesson F3.6 or when asked to do F3.6.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F3.6
  commons-audit: programme/conditional-approval-design
...
The learner wants to do lesson **F3.6**: they typed `/f3-6-saying-no`, picked it from
their agent's skill list, or asked to "do F3.6". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F3.6`. It gets the CyberTravels code as it stood at
   the end of F3.6 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/programme/conditional-approval-design`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/programme/conditional-approval-design`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F3.6 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Take a request you refused in the last year and write the five conditions that would have made it a yes. Send them to the team that asked — they will usually accept, and you get the visibility you lost by refusing.

---
name: a2-2-the-audit-trail
description: >-
  A2.2 — The audit trail, and the four questions it has to answer. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of A2.2, runs it, and reports what happened. Use when working through lesson A2.2 or when asked to do A2.2.
license: Apache-2.0
...
The learner wants to do lesson **A2.2**: they typed `/a2-2-the-audit-trail`, picked it from
their agent's skill list, or asked to "do A2.2". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py A2.2`. It gets the CyberTravels code as it stood at
   the end of A2.2 into `work/cybertravels`, shows what this lesson added,
...
3. **The audit.**
   - The audit, `skills/identity/attribution-ledger-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py A2.2 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Add the motivating input and its origin to the audit row. Then re-run the check and notice that you have also just put traveller text into a long-lived store, which is F2.5's problem.

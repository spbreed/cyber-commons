---
name: b3-11-securing-the-developers-coding-agents
description: >-
  B3.11 — Securing the developers' coding agents. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B3.11, runs it, and reports what happened. Use when working through lesson B3.11 or when asked to do B3.11.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B3.11
  commons-audit: appsec/coding-agent-hardening
...
The learner wants to do lesson **B3.11**: they typed `/b3-11-securing-the-developers-coding-agents`, picked it from
their agent's skill list, or asked to "do B3.11". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B3.11`. It gets the CyberTravels code as it stood at
   the end of B3.11 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/appsec/coding-agent-hardening`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/appsec/coding-agent-hardening`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B3.11 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Ship the credential deny-list first — it is a config file, it takes an afternoon, and no developer will notice. Then find out how many agents in your organisation could read `~/.aws/credentials` yesterday.

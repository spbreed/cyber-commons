---
name: b2-7-attribution-an-audit-trail-that-answers
description: >-
  B2.7 — Attribution: an audit trail that answers "who". Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B2.7, runs it, and reports what happened. Use when working through lesson B2.7 or when asked to do B2.7.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B2.7
  commons-audit: identity/attribution-ledger-check
...
The learner wants to do lesson **B2.7**: they typed `/b2-7-attribution-an-audit-trail-that-answers`, picked it from
their agent's skill list, or asked to "do B2.7". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B2.7`. It gets the CyberTravels code as it stood at
   the end of B2.7 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/identity/attribution-ledger-check`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/identity/attribution-ledger-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B2.7 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Take the last significant action one of your agents performed and try to fill in these four fields from what you actually logged. The missing one is almost always the motivating input.

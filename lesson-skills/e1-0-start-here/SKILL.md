---
name: e1-0-start-here
description: >-
  E1.0 — Start here. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E1.0, runs it, and reports what happened. Use when working through lesson E1.0 or when asked to do E1.0.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E1.0
  commons-audit: detection/agent-tempo-baseline
...
The learner wants to do lesson **E1.0**: they typed `/e1-0-start-here`, picked it from
their agent's skill list, or asked to "do E1.0". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E1.0`. It gets the CyberTravels code as it stood at
   the end of E1.0 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/detection/agent-tempo-baseline`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/detection/agent-tempo-baseline`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E1.0 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Pull one hour of activity for a service account in your own environment and compute those five signals. If you cannot, that is the first finding of chapter 8 and it is a telemetry problem rather than a detection one.

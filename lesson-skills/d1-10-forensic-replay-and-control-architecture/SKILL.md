---
name: d1-10-forensic-replay-and-control-architecture
description: >-
  D1.10 — Forensic replay and control architecture. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of D1.10, runs it, and reports what happened. Use when working through lesson D1.10 or when asked to do D1.10.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: D1.10
  commons-audit: response/run-replayability-audit
...
The learner wants to do lesson **D1.10**: they typed `/d1-10-forensic-replay-and-control-architecture`, picked it from
their agent's skill list, or asked to "do D1.10". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py D1.10`. It gets the CyberTravels code as it stood at
   the end of D1.10 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/response/run-replayability-audit`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/response/run-replayability-audit`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py D1.10 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Pick one agent run from last week and try to reproduce it. The first field you cannot recover is the one to start logging today.

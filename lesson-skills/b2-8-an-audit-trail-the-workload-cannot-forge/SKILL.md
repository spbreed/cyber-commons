---
name: b2-8-an-audit-trail-the-workload-cannot-forge
description: >-
  B2.8 — An audit trail the workload cannot forge. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B2.8, runs it, and reports what happened. Use when working through lesson B2.8 or when asked to do B2.8.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B2.8
  commons-audit: identity/tamper-evident-log-check
...
The learner wants to do lesson **B2.8**: they typed `/b2-8-an-audit-trail-the-workload-cannot-forge`, picked it from
their agent's skill list, or asked to "do B2.8". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B2.8`. It gets the CyberTravels code as it stood at
   the end of B2.8 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/identity/tamper-evident-log-check`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/identity/tamper-evident-log-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B2.8 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Run the IAM check on your own agent workload: can its role read, write or list the bucket its transcripts land in, or decrypt with their key? If yes, every detection you have built on those transcripts is reporting on data the subject controls.

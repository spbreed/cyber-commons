---
name: c2-6-sandbox-replication
description: >-
  C2.6 — Sandbox replication. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of C2.6, runs it, and reports what happened. Use when working through lesson C2.6 or when asked to do C2.6.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: C2.6
  commons-audit: appsec/exploit-replica-isolation-check
...
The learner wants to do lesson **C2.6**: they typed `/c2-6-sandbox-replication`, picked it from
their agent's skill list, or asked to "do C2.6". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py C2.6`. It gets the CyberTravels code as it stood at
   the end of C2.6 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/appsec/exploit-replica-isolation-check`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/appsec/exploit-replica-isolation-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py C2.6 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Check whether your dynamic testing currently runs against staging. If it does, list what staging shares with production — identity provider, message bus, data replica. Each shared component is a path from a test probe to a real incident.

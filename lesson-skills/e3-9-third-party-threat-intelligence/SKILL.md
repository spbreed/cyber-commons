---
name: e3-9-third-party-threat-intelligence
description: >-
  E3.9 — Third-party threat intelligence, and the tactics it names. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E3.9, runs it, and reports what happened. Use when working through lesson E3.9 or when asked to do E3.9.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E3.9
  commons-audit: detection/threat-intel-to-rules
...
The learner wants to do lesson **E3.9**: they typed `/e3-9-third-party-threat-intelligence`, picked it from
their agent's skill list, or asked to "do E3.9". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E3.9`. It gets the CyberTravels code as it stood at
   the end of E3.9 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/detection/threat-intel-to-rules`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/detection/threat-intel-to-rules`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E3.9 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Compute your own three numbers for last quarter: indicators received, rules deployed, alerts actioned. The ratio between the first and third is the honest value of the programme.

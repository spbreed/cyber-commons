---
name: b1-14-repudiation-and-untraceability
description: >-
  B1.14 — Repudiation and untraceability. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B1.14, runs it, and reports what happened. Use when working through lesson B1.14 or when asked to do B1.14.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B1.14
  commons-audit: threats/audit-answerability-check
...
The learner wants to do lesson **B1.14**: they typed `/b1-14-repudiation-and-untraceability`, picked it from
their agent's skill list, or asked to "do B1.14". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B1.14`. It gets the CyberTravels code as it stood at
   the end of B1.14 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/threats/audit-answerability-check`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/threats/audit-answerability-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B1.14 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Take yesterday's agent logs and try to answer 'which user caused this action'. Time how long it takes. That number is your time-to-attribution during an incident, when it will be worse.

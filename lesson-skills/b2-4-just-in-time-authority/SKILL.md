---
name: b2-4-just-in-time-authority
description: >-
  B2.4 — Just-in-time authority. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B2.4, runs it, and reports what happened. Use when working through lesson B2.4 or when asked to do B2.4.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B2.4
  commons-audit: attestation/entitlement-overprivilege-analyzer
...
The learner wants to do lesson **B2.4**: they typed `/b2-4-just-in-time-authority`, picked it from
their agent's skill list, or asked to "do B2.4". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B2.4`. It gets the CyberTravels code as it stood at
   the end of B2.4 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/attestation/entitlement-overprivilege-analyzer`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/attestation/entitlement-overprivilege-analyzer`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B2.4 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Take one standing grant an agent holds and work out what would break if it expired in two minutes. That list is the real cost of just-in-time, and it is usually shorter than expected.

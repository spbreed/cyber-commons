---
name: f1-3-risk-tiering-agentic-use-cases
description: >-
  F1.3 — Risk tiering agentic use cases. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F1.3, runs it, and reports what happened. Use when working through lesson F1.3 or when asked to do F1.3.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F1.3
  commons-audit: grc/agentic-risk-tiering
...
The learner wants to do lesson **F1.3**: they typed `/f1-3-risk-tiering-agentic-use-cases`, picked it from
their agent's skill list, or asked to "do F1.3". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F1.3`. It gets the CyberTravels code as it stood at
   the end of F1.3 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/grc/agentic-risk-tiering`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/grc/agentic-risk-tiering`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F1.3 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Re-tier your top ten AI use cases using only the four questions. Note which ones move, and be ready to explain the movement to whoever wrote the original questionnaire — the model question is usually question one.

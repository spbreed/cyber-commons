---
name: f3-2-governing-autonomy-rather-than-approving
description: >-
  F3.2 — Governing autonomy rather than approving tools. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F3.2, runs it, and reports what happened. Use when working through lesson F3.2 or when asked to do F3.2.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F3.2
  commons-audit: programme/autonomy-ladder-decisions
...
The learner wants to do lesson **F3.2**: they typed `/f3-2-governing-autonomy-rather-than-approving`, picked it from
their agent's skill list, or asked to "do F3.2". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F3.2`. It gets the CyberTravels code as it stood at
   the end of F3.2 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/programme/autonomy-ladder-decisions`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/programme/autonomy-ladder-decisions`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F3.2 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Write your own per-rung policy in four lines and check what L1 costs a team today. If registering a read-only copilot needs an approval, your inventory is already incomplete and you cannot see by how much.

...
name: a1-5-memory
description: >-
  A1.5 — Memory. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of A1.5, runs it, and reports what happened. Use when working through lesson A1.5 or when asked to do A1.5.
license: Apache-2.0
...
The learner wants to do lesson **A1.5**: they typed `/a1-5-memory`, picked it from
their agent's skill list, or asked to "do A1.5". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py A1.5`. It gets the CyberTravels code as it stood at
   the end of A1.5 into `work/cybertravels`, shows what this lesson added,
...
3. **The audit.**
   - The audit, `skills/threats/memory-scope-and-origin-audit`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py A1.5 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Drop the origin column and re-render the prompt block. The vendor's instruction now reads exactly like the policy. Nothing errored, and nothing will.

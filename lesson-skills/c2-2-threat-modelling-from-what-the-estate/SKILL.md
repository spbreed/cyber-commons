---
name: c2-2-threat-modelling-from-what-the-estate
description: >-
  C2.2 — Threat modelling from what the estate already knows. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of C2.2, runs it, and reports what happened. Use when working through lesson C2.2 or when asked to do C2.2.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: C2.2
  commons-audit: appsec/threat-model-stride, research/generated-schema-audit
...
The learner wants to do lesson **C2.2**: they typed `/c2-2-threat-modelling-from-what-the-estate`, picked it from
their agent's skill list, or asked to "do C2.2". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py C2.2`. It gets the CyberTravels code as it stood at
   the end of C2.2 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/appsec/threat-model-stride`, `skills/research/generated-schema-audit`). It ends with a READBACK.
3. **The audit.**
   - Audit 1 of 2, `skills/appsec/threat-model-stride`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py C2.2 --audit 1 --answer work/answer.json`.
   - Audit 2 of 2, `skills/research/generated-schema-audit`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py C2.2 --audit 2 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Point the skill at one of your own services. The work is not the model, it is collecting the five inputs: if any of them is "in somebody's head", that is the input your threat model is currently guessing at, and the guess is always the optimistic one.

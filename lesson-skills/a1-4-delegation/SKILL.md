---
name: a1-4-delegation
description: >-
  A1.4 — Delegation. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of A1.4, runs it, and reports what happened. Use when working through lesson A1.4 or when asked to do A1.4.
license: Apache-2.0
...
The learner wants to do lesson **A1.4**: they typed `/a1-4-delegation`, picked it from
their agent's skill list, or asked to "do A1.4". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py A1.4`. It gets the CyberTravels code as it stood at
   the end of A1.4 into `work/cybertravels`, shows what this lesson added,
...
3. **The audit.**
   - The audit, `skills/attestation/identity-chain-verifier`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py A1.4 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Take a token minted for the internal server and present it to the vendor server. Read the refusal. Then remove the audience check and watch the same token work in both places.

---
name: c2-4-deduplication-and-contextual
description: >-
  C2.4 — Deduplication and contextual verification. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of C2.4, runs it, and reports what happened. Use when working through lesson C2.4 or when asked to do C2.4.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: C2.4
  commons-audit: appsec/finding-dedup-and-verification
...
The learner wants to do lesson **C2.4**: they typed `/c2-4-deduplication-and-contextual`, picked it from
their agent's skill list, or asked to "do C2.4". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py C2.4`. It gets the CyberTravels code as it stood at
   the end of C2.4 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/appsec/finding-dedup-and-verification`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/appsec/finding-dedup-and-verification`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py C2.4 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Add a fourth verification check: does the CWE class match the sink type? A CWE-22 finding on a `conn.execute` call is provably mislabelled, and that check costs nothing to run.

---
name: f1-2-building-the-ai-and-agent-inventory
description: >-
  F1.2 — Building the AI and agent inventory. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F1.2, runs it, and reports what happened. Use when working through lesson F1.2 or when asked to do F1.2.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F1.2
  commons-audit: attestation/deployment-inventory-resolver
...
The learner wants to do lesson **F1.2**: they typed `/f1-2-building-the-ai-and-agent-inventory`, picked it from
their agent's skill list, or asked to "do F1.2". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F1.2`. It gets the CyberTravels code as it stood at
   the end of F1.2 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/attestation/deployment-inventory-resolver`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/attestation/deployment-inventory-resolver`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F1.2 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Run the egress query for real: one week of traffic to model-provider domains, joined against your inventory. It takes an hour and it always finds something.

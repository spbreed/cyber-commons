---
name: b3-5-validating-what-comes-back
description: >-
  B3.5 — Validating what comes back. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B3.5, runs it, and reports what happened. Use when working through lesson B3.5 or when asked to do B3.5.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B3.5
  commons-audit: runtime/tool-return-validation-check
...
The learner wants to do lesson **B3.5**: they typed `/b3-5-validating-what-comes-back`, picked it from
their agent's skill list, or asked to "do B3.5". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B3.5`. It gets the CyberTravels code as it stood at
   the end of B3.5 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/runtime/tool-return-validation-check`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/runtime/tool-return-validation-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B3.5 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Find one place a sub-agent's output becomes another agent's input and ask what oracle checks it. If the answer is the model's own confidence, that is the component grading its own work.

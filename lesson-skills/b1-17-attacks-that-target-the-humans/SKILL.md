---
name: b1-17-attacks-that-target-the-humans
description: >-
  B1.17 — Attacks that target the humans. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B1.17, runs it, and reports what happened. Use when working through lesson B1.17 or when asked to do B1.17.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B1.17
  commons-audit: threats/authority-composition-check
...
The learner wants to do lesson **B1.17**: they typed `/b1-17-attacks-that-target-the-humans`, picked it from
their agent's skill list, or asked to "do B1.17". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B1.17`. It gets the CyberTravels code as it stood at
   the end of B1.17 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/threats/authority-composition-check`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/threats/authority-composition-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B1.17 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Take one permission a user is denied and see whether an agent they can talk to holds it. That pair is a laundering path, and it is invisible to any review that checks permissions one hop at a time.

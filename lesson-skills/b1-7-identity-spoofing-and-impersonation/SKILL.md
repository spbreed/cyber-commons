---
name: b1-7-identity-spoofing-and-impersonation
description: >-
  B1.7 — Identity spoofing and impersonation. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B1.7, runs it, and reports what happened. Use when working through lesson B1.7 or when asked to do B1.7.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B1.7
  commons-audit: threats/shared-credential-attribution-check
...
The learner wants to do lesson **B1.7**: they typed `/b1-7-identity-spoofing-and-impersonation`, picked it from
their agent's skill list, or asked to "do B1.7". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B1.7`. It gets the CyberTravels code as it stood at
   the end of B1.7 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/threats/shared-credential-attribution-check`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/threats/shared-credential-attribution-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B1.7 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Count the distinct credentials across your agents and divide by the number of agents. Any answer below one is this risk, and the number tells you how many innocent agents a revocation takes down.

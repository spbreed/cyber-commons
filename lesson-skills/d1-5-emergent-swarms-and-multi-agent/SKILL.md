---
name: d1-5-emergent-swarms-and-multi-agent
description: >-
  D1.5 — Emergent swarms and multi-agent proliferation. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of D1.5, runs it, and reports what happened. Use when working through lesson D1.5 or when asked to do D1.5.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: D1.5
  commons-audit: research/incident-control-mapping
...
The learner wants to do lesson **D1.5**: they typed `/d1-5-emergent-swarms-and-multi-agent`, picked it from
their agent's skill list, or asked to "do D1.5". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py D1.5`. It gets the CyberTravels code as it stood at
   the end of D1.5 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/research/incident-control-mapping`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/research/incident-control-mapping`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py D1.5 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Ask whether anything in your estate could spawn a child agent. If the answer is yes and you cannot attribute the child to a human, you already have the swarm's precondition.

---
name: b3-8-shared-infrastructure-between-agent-runs
description: >-
  B3.8 — Shared infrastructure between agent runs. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B3.8, runs it, and reports what happened. Use when working through lesson B3.8 or when asked to do B3.8.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B3.8
  commons-audit: runtime/shared-surface-channel-audit
...
The learner wants to do lesson **B3.8**: they typed `/b3-8-shared-infrastructure-between-agent-runs`, picked it from
their agent's skill list, or asked to "do B3.8". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B3.8`. It gets the CyberTravels code as it stood at
   the end of B3.8 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/runtime/shared-surface-channel-audit`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/runtime/shared-surface-channel-audit`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B3.8 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** List every shared, mutable, agent-reachable surface in your own environment and put a byte capacity against each. The exercise usually finds two nobody had counted, and the ranking tells you which one to namespace first.

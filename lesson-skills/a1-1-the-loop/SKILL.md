...
name: a1-1-the-loop
description: >-
  A1.1 — The loop. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of A1.1, runs it, and reports what happened. Use when working through lesson A1.1 or when asked to do A1.1.
license: Apache-2.0
...
The learner wants to do lesson **A1.1**: they typed `/a1-1-the-loop`, picked it from
their agent's skill list, or asked to "do A1.1". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py A1.1`. It gets the CyberTravels code as it stood at
   the end of A1.1 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/appsec/agentic-harness-loop`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/appsec/agentic-harness-loop`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py A1.1 --answer work/answer.json`.

---
name: f1-11-model-risk-management-for-ai-systems
description: >-
  F1.11 — Model risk management for AI systems. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F1.11, runs it, and reports what happened. Use when working through lesson F1.11 or when asked to do F1.11.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F1.11
  commons-audit: grc/model-risk-validation-scope, research/weight-access-capability-delta
...
The learner wants to do lesson **F1.11**: they typed `/f1-11-model-risk-management-for-ai-systems`, picked it from
their agent's skill list, or asked to "do F1.11". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F1.11`. It gets the CyberTravels code as it stood at
   the end of F1.11 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/grc/model-risk-validation-scope`, `skills/research/weight-access-capability-delta`). It ends with a READBACK.
3. **The audit.**
   - Audit 1 of 2, `skills/grc/model-risk-validation-scope`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F1.11 --audit 1 --answer work/answer.json`.
   - Audit 2 of 2, `skills/research/weight-access-capability-delta`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F1.11 --audit 2 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Take one validated model in your estate and list the tools it holds today. If any of them post-dates the validation report, the report is describing a different system.

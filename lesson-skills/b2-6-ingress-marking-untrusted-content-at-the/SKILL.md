---
name: b2-6-ingress-marking-untrusted-content-at-the
description: >-
  B2.6 — Ingress: marking untrusted content at the door. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B2.6, runs it, and reports what happened. Use when working through lesson B2.6 or when asked to do B2.6.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B2.6
  commons-audit: attestation/input-injection-screening-verifier
...
The learner wants to do lesson **B2.6**: they typed `/b2-6-ingress-marking-untrusted-content-at-the`, picked it from
their agent's skill list, or asked to "do B2.6". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B2.6`. It gets the CyberTravels code as it stood at
   the end of B2.6 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/attestation/input-injection-screening-verifier`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/attestation/input-injection-screening-verifier`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B2.6 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** List every place text enters your agent's context and check which of them attaches an origin. The untagged ones are the paths where this control does not exist, whatever the design document says.

---
name: f1-12-working-the-seams
description: >-
  F1.12 — Working the seams. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F1.12, runs it, and reports what happened. Use when working through lesson F1.12 or when asked to do F1.12.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F1.12
  commons-audit: programme/handoff-delivery-check
...
The learner wants to do lesson **F1.12**: they typed `/f1-12-working-the-seams`, picked it from
their agent's skill list, or asked to "do F1.12". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F1.12`. It gets the CyberTravels code as it stood at
   the end of F1.12 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/programme/handoff-delivery-check`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/programme/handoff-delivery-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F1.12 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Pick the artefact your function produces for someone else and ask the recipient when they last received it. The gap between 'we produce that' and 'we receive that' is the seam, and it is usually measured in quarters.

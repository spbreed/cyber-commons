---
name: f2-7-documentation-that-survives-supervision
description: >-
  F2.7 — Documentation that survives supervision. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F2.7, runs it, and reports what happened. Use when working through lesson F2.7 or when asked to do F2.7.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F2.7
  commons-audit: regulatory/supervisory-documentation-score
...
The learner wants to do lesson **F2.7**: they typed `/f2-7-documentation-that-survives-supervision`, picked it from
their agent's skill list, or asked to "do F2.7". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F2.7`. It gets the CyberTravels code as it stood at
   the end of F2.7 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/regulatory/supervisory-documentation-score`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/regulatory/supervisory-documentation-score`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F2.7 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Rewrite one paragraph of your AI policy in the strong shape. Any sentence that cannot name an artefact is intent — label it as such rather than deleting it, and the document gets more credible.

---
name: b1-16-misaligned-and-deceptive-behaviour
description: >-
  B1.16 — Misaligned and deceptive behaviour. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B1.16, runs it, and reports what happened. Use when working through lesson B1.16 or when asked to do B1.16.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B1.16
  commons-audit: threats/objective-gaming-check
...
The learner wants to do lesson **B1.16**: they typed `/b1-16-misaligned-and-deceptive-behaviour`, picked it from
their agent's skill list, or asked to "do B1.16". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B1.16`. It gets the CyberTravels code as it stood at
   the end of B1.16 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/threats/objective-gaming-check`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/threats/objective-gaming-check`, is a **worked example with no model in it**. The harness runs it itself and its output is the result. Do not answer anything for it.
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Write down the objective one of your agents optimises and then write the cheapest way to satisfy that sentence without doing the work. If you can find one in under a minute, so can the loop.

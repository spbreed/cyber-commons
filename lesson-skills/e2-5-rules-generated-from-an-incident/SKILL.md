---
name: e2-5-rules-generated-from-an-incident
description: >-
  E2.5 — Rules generated from an incident. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E2.5, runs it, and reports what happened. Use when working through lesson E2.5 or when asked to do E2.5.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E2.5
  commons-audit: detection/detection-rule-synthesis
...
The learner wants to do lesson **E2.5**: they typed `/e2-5-rules-generated-from-an-incident`, picked it from
their agent's skill list, or asked to "do E2.5". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E2.5`. It gets the CyberTravels code as it stood at
   the end of E2.5 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/detection/detection-rule-synthesis`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/detection/detection-rule-synthesis`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E2.5 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Add a benign run that the shipped rule fires on. If you cannot construct one, your corpus is too easy.

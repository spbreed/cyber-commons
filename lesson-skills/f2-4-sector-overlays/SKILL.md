---
name: f2-4-sector-overlays
description: >-
  F2.4 — Sector overlays. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F2.4, runs it, and reports what happened. Use when working through lesson F2.4 or when asked to do F2.4.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F2.4
  commons-audit: regulatory/sector-overlay-assessment
...
The learner wants to do lesson **F2.4**: they typed `/f2-4-sector-overlays`, picked it from
their agent's skill list, or asked to "do F2.4". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F2.4`. It gets the CyberTravels code as it stood at
   the end of F2.4 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/regulatory/sector-overlay-assessment`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/regulatory/sector-overlay-assessment`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F2.4 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Find the clause in your own sector overlay that already covers autonomous action without naming AI. Citing it is faster, cheaper and more persuasive than any new AI policy you could write.

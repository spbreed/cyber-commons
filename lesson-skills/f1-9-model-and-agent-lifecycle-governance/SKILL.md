---
name: f1-9-model-and-agent-lifecycle-governance
description: >-
  F1.9 — Model and agent lifecycle governance. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F1.9, runs it, and reports what happened. Use when working through lesson F1.9 or when asked to do F1.9.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F1.9
  commons-audit: grc/agent-lifecycle-governance
...
The learner wants to do lesson **F1.9**: they typed `/f1-9-model-and-agent-lifecycle-governance`, picked it from
their agent's skill list, or asked to "do F1.9". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F1.9`. It gets the CyberTravels code as it stood at
   the end of F1.9 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/grc/agent-lifecycle-governance`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/grc/agent-lifecycle-governance`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F1.9 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Query your identity provider for non-human identities whose service is retired but which authenticated in the last 30 days. Every hit is either an undocumented dependency or someone else's foothold, and you cannot tell which from the directory alone.

---
name: b1-15-overwhelming-the-human-in-the-loop
description: >-
  B1.15 — Overwhelming the human in the loop. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B1.15, runs it, and reports what happened. Use when working through lesson B1.15 or when asked to do B1.15.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B1.15
  commons-audit: threats/approval-queue-saturation-model
...
The learner wants to do lesson **B1.15**: they typed `/b1-15-overwhelming-the-human-in-the-loop`, picked it from
their agent's skill list, or asked to "do B1.15". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B1.15`. It gets the CyberTravels code as it stood at
   the end of B1.15 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/threats/approval-queue-saturation-model`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/threats/approval-queue-saturation-model`, is a **worked example with no model in it**. The harness runs it itself and its output is the result. Do not answer anything for it.
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Count how many approval requests one of your agents generates per day and ask the person approving them how many they read in full. The gap between those two numbers is the control's real coverage.

---
name: e3-4-when-the-actor-is-an-agent
description: >-
  E3.4 — When the actor is an agent. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E3.4, runs it, and reports what happened. Use when working through lesson E3.4 or when asked to do E3.4.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E3.4
  commons-audit: response/agent-actor-containment
...
The learner wants to do lesson **E3.4**: they typed `/e3-4-when-the-actor-is-an-agent`, picked it from
their agent's skill list, or asked to "do E3.4". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E3.4`. It gets the CyberTravels code as it stood at
   the end of E3.4 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/response/agent-actor-containment`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/response/agent-actor-containment`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E3.4 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Write your agentic incident runbook's first three steps. If step one is "disable the user account", rewrite it — and check whether you can currently revoke a single agent identity at all.

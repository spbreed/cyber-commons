---
name: e3-2-admission-rules
description: >-
  E3.2 — Admission rules. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E3.2, runs it, and reports what happened. Use when working through lesson E3.2 or when asked to do E3.2.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E3.2
  commons-audit: secops/investigation-admission-rules
...
The learner wants to do lesson **E3.2**: they typed `/e3-2-admission-rules`, picked it from
their agent's skill list, or asked to "do E3.2". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E3.2`. It gets the CyberTravels code as it stood at
   the end of E3.2 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/secops/investigation-admission-rules`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/secops/investigation-admission-rules`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E3.2 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Write the admission set for a data-exfiltration investigation. It is not the same set, and working out why is the exercise.

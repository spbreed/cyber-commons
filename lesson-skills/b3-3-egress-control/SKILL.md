---
name: b3-3-egress-control
description: >-
  B3.3 — Egress control. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B3.3, runs it, and reports what happened. Use when working through lesson B3.3 or when asked to do B3.3.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B3.3
  commons-audit: attestation/sandbox-egress-verifier
...
The learner wants to do lesson **B3.3**: they typed `/b3-3-egress-control`, picked it from
their agent's skill list, or asked to "do B3.3". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B3.3`. It gets the CyberTravels code as it stood at
   the end of B3.3 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/attestation/sandbox-egress-verifier`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/attestation/sandbox-egress-verifier`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B3.3 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Write the allow-list for one agent by listing the hosts it genuinely calls. If it is under five, you can ship this control this week; if it is unbounded, that is the finding.

---
name: e4-4-stop-authority
description: >-
  E4.4 — Stop authority. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E4.4, runs it, and reports what happened. Use when working through lesson E4.4 or when asked to do E4.4.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E4.4
  commons-audit: response/stop-authority-readiness
...
The learner wants to do lesson **E4.4**: they typed `/e4-4-stop-authority`, picked it from
their agent's skill list, or asked to "do E4.4". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E4.4`. It gets the CyberTravels code as it stood at
   the end of E4.4 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/response/stop-authority-readiness`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/response/stop-authority-readiness`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E4.4 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Run the game day. The deliverable is the number, and the number is what goes in the evidence pack for F1.7 and the board slide for F3.5. An untested stop button is a belief.

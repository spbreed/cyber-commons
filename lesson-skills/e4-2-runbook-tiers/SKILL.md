---
name: e4-2-runbook-tiers
description: >-
  E4.2 — Runbook tiers. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E4.2, runs it, and reports what happened. Use when working through lesson E4.2 or when asked to do E4.2.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E4.2
  commons-audit: response/runbook-tier-assignment
...
The learner wants to do lesson **E4.2**: they typed `/e4-2-runbook-tiers`, picked it from
their agent's skill list, or asked to "do E4.2". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E4.2`. It gets the CyberTravels code as it stood at
   the end of E4.2 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/response/runbook-tier-assignment`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/response/runbook-tier-assignment`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E4.2 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Put your own detection's false-positive rate in and see whether the tier you already ship still looks right.

---
name: d1-6-high-concurrency-detection-engineering
description: >-
  D1.6 — High-concurrency detection engineering. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of D1.6, runs it, and reports what happened. Use when working through lesson D1.6 or when asked to do D1.6.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: D1.6
  commons-audit: detection/detection-rule-deployability
...
The learner wants to do lesson **D1.6**: they typed `/d1-6-high-concurrency-detection-engineering`, picked it from
their agent's skill list, or asked to "do D1.6". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py D1.6`. It gets the CyberTravels code as it stood at
   the end of D1.6 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/detection/detection-rule-deployability`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/detection/detection-rule-deployability`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py D1.6 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Take a detection you are proud of and compute how many times it fired last month against how many were true. If you cannot, the rule is unmeasured, which is the same as untuned.

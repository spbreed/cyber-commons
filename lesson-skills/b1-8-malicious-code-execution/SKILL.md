---
name: b1-8-malicious-code-execution
description: >-
  B1.8 — Malicious code execution. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B1.8, runs it, and reports what happened. Use when working through lesson B1.8 or when asked to do B1.8.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B1.8
  commons-audit: threats/generated-code-reach-enumerator
...
The learner wants to do lesson **B1.8**: they typed `/b1-8-malicious-code-execution`, picked it from
their agent's skill list, or asked to "do B1.8". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B1.8`. It gets the CyberTravels code as it stood at
   the end of B1.8 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/threats/generated-code-reach-enumerator`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/threats/generated-code-reach-enumerator`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B1.8 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** For one agent that executes code, list what is in its process environment right now. The credentials in that list are the blast radius of the next ordinary bug, not of the next attack.

---
name: a1-0-what-an-agent-is
description: >-
  A1.0 — What an agent is, and what you are about to build. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of A1.0, runs it, and reports what happened. Use when working through lesson A1.0 or when asked to do A1.0.
license: Apache-2.0
...
The learner wants to do lesson **A1.0**: they typed `/a1-0-what-an-agent-is`, picked it from
their agent's skill list, or asked to "do A1.0". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py A1.0`. It gets the CyberTravels code as it stood at
   the end of A1.0 into `work/cybertravels`, shows what this lesson added,
...
3. **The audit.**
   - The audit, `skills/architecture/agentic-architecture-map`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py A1.0 --answer work/answer.json`.

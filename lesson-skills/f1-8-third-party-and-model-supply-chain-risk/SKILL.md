---
name: f1-8-third-party-and-model-supply-chain-risk
description: >-
  F1.8 — Third-party and model supply chain risk. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F1.8, runs it, and reports what happened. Use when working through lesson F1.8 or when asked to do F1.8.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F1.8
  commons-audit: grc/third-party-ai-assessment
...
The learner wants to do lesson **F1.8**: they typed `/f1-8-third-party-and-model-supply-chain-risk`, picked it from
their agent's skill list, or asked to "do F1.8". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F1.8`. It gets the CyberTravels code as it stood at
   the end of F1.8 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/grc/third-party-ai-assessment`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/grc/third-party-ai-assessment`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F1.8 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Add "can this change without notifying us?" to your third-party assessment form. For hosted models the answer is usually yes, and it should carry an explicit control-test expiry.

---
name: b3-9-turning-a-control-off-without-turning-th
description: >-
  B3.9 — Turning a control off without turning the system into an experiment. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B3.9, runs it, and reports what happened. Use when working through lesson B3.9 or when asked to do B3.9.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B3.9
  commons-audit: runtime/control-exemption-audit
...
The learner wants to do lesson **B3.9**: they typed `/b3-9-turning-a-control-off-without-turning-th`, picked it from
their agent's skill list, or asked to "do B3.9". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B3.9`. It gets the CyberTravels code as it stood at
   the end of B3.9 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/runtime/control-exemption-audit`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/runtime/control-exemption-audit`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B3.9 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Find one control currently disabled in your environment and ask three questions: who approved it, when does it expire, and what cap came with it. The third question is the one that usually has no answer.

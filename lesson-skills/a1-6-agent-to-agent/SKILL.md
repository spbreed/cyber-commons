---
name: a1-6-agent-to-agent
description: >-
  A1.6 — Agent to agent. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of A1.6, runs it, and reports what happened. Use when working through lesson A1.6 or when asked to do A1.6.
license: Apache-2.0
...
The learner wants to do lesson **A1.6**: they typed `/a1-6-agent-to-agent`, picked it from
their agent's skill list, or asked to "do A1.6". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py A1.6`. It gets the CyberTravels code as it stood at
   the end of A1.6 into `work/cybertravels`, shows what this lesson added,
...
3. **The audit.**
   - The audit, `skills/threats/peer-message-propagation-trace`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py A1.6 --answer work/answer.json`.

---
name: e1-2-drift-monitoring
description: >-
  E1.2 — Drift monitoring. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E1.2, runs it, and reports what happened. Use when working through lesson E1.2 or when asked to do E1.2.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E1.2
  commons-audit: detection/behavioural-drift-monitor
...
The learner wants to do lesson **E1.2**: they typed `/e1-2-drift-monitoring`, picked it from
their agent's skill list, or asked to "do E1.2". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E1.2`. It gets the CyberTravels code as it stood at
   the end of E1.2 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/detection/behavioural-drift-monitor`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/detection/behavioural-drift-monitor`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E1.2 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Compute the drift rate for one production agent from three months of telemetry, and set its control freshness window from that number rather than from the audit calendar.

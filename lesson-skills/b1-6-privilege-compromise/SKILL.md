---
name: b1-6-privilege-compromise
description: >-
  B1.6 — Privilege compromise. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B1.6, runs it, and reports what happened. Use when working through lesson B1.6 or when asked to do B1.6.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B1.6
  commons-audit: threats/authorization-subject-check
...
The learner wants to do lesson **B1.6**: they typed `/b1-6-privilege-compromise`, picked it from
their agent's skill list, or asked to "do B1.6". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B1.6`. It gets the CyberTravels code as it stood at
   the end of B1.6 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/threats/authorization-subject-check`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/threats/authorization-subject-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B1.6 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Pick one agent and answer two questions: what identity does it authenticate as, and can you name the human behind any single action it took last week. If the second answer is no, you have this risk regardless of how the scopes are set.

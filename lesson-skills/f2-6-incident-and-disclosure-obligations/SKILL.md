---
name: f2-6-incident-and-disclosure-obligations
description: >-
  F2.6 — Incident and disclosure obligations. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F2.6, runs it, and reports what happened. Use when working through lesson F2.6 or when asked to do F2.6.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F2.6
  commons-audit: regulatory/disclosure-phase-breakdown
...
The learner wants to do lesson **F2.6**: they typed `/f2-6-incident-and-disclosure-obligations`, picked it from
their agent's skill list, or asked to "do F2.6". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F2.6`. It gets the CyberTravels code as it stood at
   the end of F2.6 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/regulatory/disclosure-phase-breakdown`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/regulatory/disclosure-phase-breakdown`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F2.6 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Draft the disclosure sentence you would send when you know an agent acted but cannot say which one. Getting legal to agree that wording takes weeks in peacetime and is impossible at hour 60.

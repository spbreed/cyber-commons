---
name: e4-1-remediation-policy
description: >-
  E4.1 — Remediation policy. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E4.1, runs it, and reports what happened. Use when working through lesson E4.1 or when asked to do E4.1.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E4.1
  commons-audit: response/remediation-policy-check
...
The learner wants to do lesson **E4.1**: they typed `/e4-1-remediation-policy`, picked it from
their agent's skill list, or asked to "do E4.1". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E4.1`. It gets the CyberTravels code as it stood at
   the end of E4.1 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/response/remediation-policy-check`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/response/remediation-policy-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E4.1 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Add an action your team performs during an incident. If you cannot answer 'reversible without a human', that is the finding.

---
name: f2-1-the-regulatory-map
description: >-
  F2.1 — The regulatory map. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F2.1, runs it, and reports what happened. Use when working through lesson F2.1 or when asked to do F2.1.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F2.1
  commons-audit: regulatory/obligation-mapping
...
The learner wants to do lesson **F2.1**: they typed `/f2-1-the-regulatory-map`, picked it from
their agent's skill list, or asked to "do F2.1". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F2.1`. It gets the CyberTravels code as it stood at
   the end of F2.1 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/regulatory/obligation-mapping`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/regulatory/obligation-mapping`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F2.1 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Build the shortest-clock register for your highest-tier AI system. Most teams discover the binding deadline is a sector overlay or a customer contract, not the AI regulation they have been reading.

---
name: c2-15-severity-calibration
description: >-
  C2.15 — Severity calibration, triaging and reporting. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of C2.15, runs it, and reports what happened. Use when working through lesson C2.15 or when asked to do C2.15.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: C2.15
  commons-audit: appsec/appsec-triage-report
...
The learner wants to do lesson **C2.15**: they typed `/c2-15-severity-calibration`, picked it from
their agent's skill list, or asked to "do C2.15". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py C2.15`. It gets the CyberTravels code as it stood at
   the end of C2.15 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/appsec/appsec-triage-report`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/appsec/appsec-triage-report`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py C2.15 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Recalculate severity for your current open findings using confirmation and reachability alone — you do not need chaining to see the effect. The queue reorders, and the items that fall are usually the ones people have been arguing about.

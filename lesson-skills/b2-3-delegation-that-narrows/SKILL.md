---
name: b2-3-delegation-that-narrows
description: >-
  B2.3 — Delegation that narrows, and survives audit. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B2.3, runs it, and reports what happened. Use when working through lesson B2.3 or when asked to do B2.3.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B2.3
  commons-audit: attestation/identity-chain-verifier
...
The learner wants to do lesson **B2.3**: they typed `/b2-3-delegation-that-narrows`, picked it from
their agent's skill list, or asked to "do B2.3". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B2.3`. It gets the CyberTravels code as it stood at
   the end of B2.3 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/attestation/identity-chain-verifier`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/attestation/identity-chain-verifier`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B2.3 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Find your token exchange and check three things: does it set an `act` claim, does it check the actor's ceiling as well as the subset rule, and does anything downstream look at `cnf`? Most implementations do the subset rule only — it is the one the specification example shows.

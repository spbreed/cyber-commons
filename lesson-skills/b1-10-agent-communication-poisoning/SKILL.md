---
name: b1-10-agent-communication-poisoning
description: >-
  B1.10 — Agent communication poisoning. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B1.10, runs it, and reports what happened. Use when working through lesson B1.10 or when asked to do B1.10.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B1.10
  commons-audit: threats/peer-message-propagation-trace
...
The learner wants to do lesson **B1.10**: they typed `/b1-10-agent-communication-poisoning`, picked it from
their agent's skill list, or asked to "do B1.10". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B1.10`. It gets the CyberTravels code as it stood at
   the end of B1.10 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/threats/peer-message-propagation-trace`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/threats/peer-message-propagation-trace`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B1.10 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** In your own multi-agent system, find where one agent's output becomes another's input and ask what validates it. If the answer is 'it came from our own agent', that is the trust asymmetry, stated.

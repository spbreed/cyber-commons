---
name: e3-5-agent-assisted-reconstruction
description: >-
  E3.5 — Agent-assisted reconstruction. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E3.5, runs it, and reports what happened. Use when working through lesson E3.5 or when asked to do E3.5.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E3.5
  commons-audit: response/incident-reconstruction-check
...
The learner wants to do lesson **E3.5**: they typed `/e3-5-agent-assisted-reconstruction`, picked it from
their agent's skill list, or asked to "do E3.5". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E3.5`. It gets the CyberTravels code as it stood at
   the end of E3.5 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/response/incident-reconstruction-check`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/response/incident-reconstruction-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E3.5 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Take a real incident timeline from your own history and ask what it would look like if an agent had been operating on the user's credential. If you cannot tell from the logs, your reconstructions already carry this risk.

---
name: d1-7-triaging-the-non-deterministic-swarm
description: >-
  D1.7 — Triaging the non-deterministic swarm. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of D1.7, runs it, and reports what happened. Use when working through lesson D1.7 or when asked to do D1.7.
license: Apache-2.0
...
The learner wants to do lesson **D1.7**: they typed `/d1-7-triaging-the-non-deterministic-swarm`, picked it from
their agent's skill list, or asked to "do D1.7". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py D1.7`. It gets the CyberTravels code as it stood at
   the end of D1.7 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/secops/detection-triage`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/secops/detection-triage`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py D1.7 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Ask your SOC whether anyone checks, when an incident is confirmed, whether an earlier alert about it was auto-closed. If not, your loop's error rate is invisible.

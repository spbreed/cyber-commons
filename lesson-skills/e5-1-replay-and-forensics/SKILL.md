---
name: e5-1-replay-and-forensics
description: >-
  E5.1 — Replay and forensics. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E5.1, runs it, and reports what happened. Use when working through lesson E5.1 or when asked to do E5.1.
license: Apache-2.0
...
The learner wants to do lesson **E5.1**: they typed `/e5-1-replay-and-forensics`, picked it from
their agent's skill list, or asked to "do E5.1". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E5.1`. It gets the CyberTravels code as it stood at
   the end of E5.1 into `work/cybertravels`, shows what this lesson added,
...
3. **The audit.**
   - The audit, `skills/response/run-replayability-audit`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E5.1 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Add model version and seed to your agent's run records this week. Both are one field each, and together they are the difference between forensics and storytelling.

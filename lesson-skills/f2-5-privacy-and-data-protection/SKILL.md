---
name: f2-5-privacy-and-data-protection
description: >-
  F2.5 — Privacy and data protection. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F2.5, runs it, and reports what happened. Use when working through lesson F2.5 or when asked to do F2.5.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F2.5
  commons-audit: regulatory/trace-personal-data-audit, research/row-level-policy-check
...
The learner wants to do lesson **F2.5**: they typed `/f2-5-privacy-and-data-protection`, picked it from
their agent's skill list, or asked to "do F2.5". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F2.5`. It gets the CyberTravels code as it stood at
   the end of F2.5 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/regulatory/trace-personal-data-audit`, `skills/research/row-level-policy-check`). It ends with a READBACK.
3. **The audit.**
   - Audit 1 of 2, `skills/regulatory/trace-personal-data-audit`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F2.5 --audit 1 --answer work/answer.json`.
   - Audit 2 of 2, `skills/research/row-level-policy-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F2.5 --audit 2 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Time-box this to an hour: can you delete one customer's data from your agent traces today? The answer usually arrives in ten minutes and is usually no — and the eval corpus is the system people forget entirely.

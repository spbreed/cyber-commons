---
name: b1-13-resource-overload
description: >-
  B1.13 — Resource overload. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B1.13, runs it, and reports what happened. Use when working through lesson B1.13 or when asked to do B1.13.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B1.13
  commons-audit: threats/unbounded-loop-cost-probe
...
The learner wants to do lesson **B1.13**: they typed `/b1-13-resource-overload`, picked it from
their agent's skill list, or asked to "do B1.13". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B1.13`. It gets the CyberTravels code as it stood at
   the end of B1.13 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/threats/unbounded-loop-cost-probe`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/threats/unbounded-loop-cost-probe`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B1.13 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Find the ceiling on one agent loop you run. If there is a token budget but no cap on downstream calls, the cost is bounded and the availability risk is not.

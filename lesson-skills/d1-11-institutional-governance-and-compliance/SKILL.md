---
name: d1-11-institutional-governance-and-compliance
description: >-
  D1.11 — Institutional governance and compliance. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of D1.11, runs it, and reports what happened. Use when working through lesson D1.11 or when asked to do D1.11.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: D1.11
  commons-audit: research/finding-to-control-handover, redteam/agentic-finding-report
...
The learner wants to do lesson **D1.11**: they typed `/d1-11-institutional-governance-and-compliance`, picked it from
their agent's skill list, or asked to "do D1.11". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py D1.11`. It gets the CyberTravels code as it stood at
   the end of D1.11 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/research/finding-to-control-handover`, `skills/redteam/agentic-finding-report`). It ends with a READBACK.
3. **The audit.**
   - Audit 1 of 2, `skills/research/finding-to-control-handover`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py D1.11 --audit 1 --answer work/answer.json`.
   - Audit 2 of 2, `skills/redteam/agentic-finding-report`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py D1.11 --audit 2 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Take your last red-team finding and write the eval case that would fail if it regressed. If you cannot, the fix is a promise, not a control.

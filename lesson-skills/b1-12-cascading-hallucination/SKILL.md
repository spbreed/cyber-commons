---
name: b1-12-cascading-hallucination
description: >-
  B1.12 — Cascading hallucination. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B1.12, runs it, and reports what happened. Use when working through lesson B1.12 or when asked to do B1.12.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B1.12
  commons-audit: threats/confidence-provenance-decay-check
...
The learner wants to do lesson **B1.12**: they typed `/b1-12-cascading-hallucination`, picked it from
their agent's skill list, or asked to "do B1.12". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B1.12`. It gets the CyberTravels code as it stood at
   the end of B1.12 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/threats/confidence-provenance-decay-check`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/threats/confidence-provenance-decay-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B1.12 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Take a finding your pipeline produced and try to walk it back to the step that first asserted it. If you cannot reach a step that checked something, you have found a cascade rather than a finding.

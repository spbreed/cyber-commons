---
name: a2-0-why-a-demo-is-not-a-system
description: >-
  A2.0 — Why a demo is not a system. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of A2.0, runs it, and reports what happened. Use when working through lesson A2.0 or when asked to do A2.0.
license: Apache-2.0
...
The learner wants to do lesson **A2.0**: they typed `/a2-0-why-a-demo-is-not-a-system`, picked it from
their agent's skill list, or asked to "do A2.0". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py A2.0`. It gets the CyberTravels code as it stood at
   the end of A2.0 into `work/cybertravels`, shows what this lesson added,
...
3. **The audit.**
   - The audit, `skills/threats/audit-answerability-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py A2.0 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Answer the same questions about a system you actually work on. The gap is usually wider than for the agent you just built, because nobody chose it.

---
name: f1-4-control-mapping-for-agents
description: >-
  F1.4 — Control mapping for agents. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F1.4, runs it, and reports what happened. Use when working through lesson F1.4 or when asked to do F1.4.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F1.4
  commons-audit: grc/control-to-framework-mapping
...
The learner wants to do lesson **F1.4**: they typed `/f1-4-control-mapping-for-agents`, picked it from
their agent's skill list, or asked to "do F1.4". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F1.4`. It gets the CyberTravels code as it stood at
   the end of F1.4 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/grc/control-to-framework-mapping`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/grc/control-to-framework-mapping`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F1.4 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Take one framework clause your programme claims to satisfy and ask which operating control produces its evidence. If the answer is a policy document, the clause is ticked and undefended.

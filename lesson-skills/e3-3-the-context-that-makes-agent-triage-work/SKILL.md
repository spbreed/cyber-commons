---
name: e3-3-the-context-that-makes-agent-triage-work
description: >-
  E3.3 — The context that makes agent triage work. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E3.3, runs it, and reports what happened. Use when working through lesson E3.3 or when asked to do E3.3.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E3.3
  commons-audit: secops/detection-triage
...
The learner wants to do lesson **E3.3**: they typed `/e3-3-the-context-that-makes-agent-triage-work`, picked it from
their agent's skill list, or asked to "do E3.3". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E3.3`. It gets the CyberTravels code as it stood at
   the end of E3.3 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/secops/detection-triage`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/secops/detection-triage`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E3.3 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Check which of the six fields your agent telemetry carries today. Scopes-held is the one almost nobody logs, and it is the one that decides the alert.

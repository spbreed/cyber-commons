---
name: e3-7-scoping-an-agentic-incident
description: >-
  E3.7 — Scoping an agentic incident. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E3.7, runs it, and reports what happened. Use when working through lesson E3.7 or when asked to do E3.7.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E3.7
  commons-audit: secops/incident-scoping
...
The learner wants to do lesson **E3.7**: they typed `/e3-7-scoping-an-agentic-incident`, picked it from
their agent's skill list, or asked to "do E3.7". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E3.7`. It gets the CyberTravels code as it stood at
   the end of E3.7 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/secops/incident-scoping`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/secops/incident-scoping`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E3.7 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** For your last incident involving a service account, recompute the scope by walking what else that account could reach. The number is almost always larger than what was written in the report.

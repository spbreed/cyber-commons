---
name: b1-18-the-cybertravels-risk-register
description: >-
  B1.18 — The CyberTravels risk register. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B1.18, runs it, and reports what happened. Use when working through lesson B1.18 or when asked to do B1.18.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B1.18
  commons-audit: attestation/risk-registry-integrator
...
The learner wants to do lesson **B1.18**: they typed `/b1-18-the-cybertravels-risk-register`, picked it from
their agent's skill list, or asked to "do B1.18". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B1.18`. It gets the CyberTravels code as it stood at
   the end of B1.18 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/attestation/risk-registry-integrator`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/attestation/risk-registry-integrator`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B1.18 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Write the same four columns for one agentic system you run. The column that will be hardest is the fourth: for each control, where is it taught, tested and evidenced in your organisation? The rows with no answer are the ones that will recur.

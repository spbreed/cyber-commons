---
name: f3-5-the-metrics-that-matter-at-your-level
description: >-
  F3.5 — The metrics that matter at your level. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F3.5, runs it, and reports what happened. Use when working through lesson F3.5 or when asked to do F3.5.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F3.5
  commons-audit: programme/programme-metrics-selection
...
The learner wants to do lesson **F3.5**: they typed `/f3-5-the-metrics-that-matter-at-your-level`, picked it from
their agent's skill list, or asked to "do F3.5". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F3.5`. It gets the CyberTravels code as it stood at
   the end of F3.5 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/programme/programme-metrics-selection`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/programme/programme-metrics-selection`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F3.5 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Which of the five can you produce today without a project? Start reporting that one monthly and let the missing ones become conspicuous — that is a cheaper way to get the others funded than asking for all five at once.

---
name: b2-2-bootstrapping-the-first-credential
description: >-
  B2.2 — Bootstrapping the first credential. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B2.2, runs it, and reports what happened. Use when working through lesson B2.2 or when asked to do B2.2.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B2.2
  commons-audit: identity/workload-attestation-check
...
The learner wants to do lesson **B2.2**: they typed `/b2-2-bootstrapping-the-first-credential`, picked it from
their agent's skill list, or asked to "do B2.2". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B2.2`. It gets the CyberTravels code as it stood at
   the end of B2.2 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/identity/workload-attestation-check`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/identity/workload-attestation-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B2.2 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Find where one of your agents gets its first credential. If the answer is an environment variable or a mounted file, list everyone who can read it — that is the set of people who are currently that agent.

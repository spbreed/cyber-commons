---
name: f3-7-building-the-capability
description: >-
  F3.7 — Building the capability. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F3.7, runs it, and reports what happened. Use when working through lesson F3.7 or when asked to do F3.7.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F3.7
  commons-audit: programme/capability-build-order
...
The learner wants to do lesson **F3.7**: they typed `/f3-7-building-the-capability`, picked it from
their agent's skill list, or asked to "do F3.7". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F3.7`. It gets the CyberTravels code as it stood at
   the end of F3.7 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/programme/capability-build-order`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/programme/capability-build-order`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F3.7 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Map your existing team onto the four roles. Most organisations have three of them under other names and are missing the identity one entirely — which is also the one that unblocks everything else.

---
name: a2-1-observability
description: >-
  A2.1 — Observability. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of A2.1, runs it, and reports what happened. Use when working through lesson A2.1 or when asked to do A2.1.
license: Apache-2.0
...
The learner wants to do lesson **A2.1**: they typed `/a2-1-observability`, picked it from
their agent's skill list, or asked to "do A2.1". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py A2.1`. It gets the CyberTravels code as it stood at
   the end of A2.1 into `work/cybertravels`, shows what this lesson added,
...
3. **The audit.**
   - The audit, `skills/response/run-replayability-audit`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py A2.1 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Put the whole delegated token in a span instead of its claims. Nothing breaks, the trace is more useful, and you have just put a credential in your log pipeline. That trade is made by accident constantly.

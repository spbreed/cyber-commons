---
name: d1-1-platform-ingestion-and-supply-chain
description: >-
  D1.1 — Platform ingestion and supply-chain risks. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of D1.1, runs it, and reports what happened. Use when working through lesson D1.1 or when asked to do D1.1.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: D1.1
  commons-audit: research/agent-supply-chain-assessment
...
The learner wants to do lesson **D1.1**: they typed `/d1-1-platform-ingestion-and-supply-chain`, picked it from
their agent's skill list, or asked to "do D1.1". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py D1.1`. It gets the CyberTravels code as it stood at
   the end of D1.1 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/research/agent-supply-chain-assessment`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/research/agent-supply-chain-assessment`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py D1.1 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** List every model and dataset your pipeline pulls at train or deploy time. The ones with no pinned digest are the ones an attacker can change without touching your code.

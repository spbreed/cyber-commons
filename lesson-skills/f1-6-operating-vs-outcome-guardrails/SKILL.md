---
name: f1-6-operating-vs-outcome-guardrails
description: >-
  F1.6 — Operating vs outcome guardrails. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F1.6, runs it, and reports what happened. Use when working through lesson F1.6 or when asked to do F1.6.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F1.6
  commons-audit: grc/guardrail-specification
...
The learner wants to do lesson **F1.6**: they typed `/f1-6-operating-vs-outcome-guardrails`, picked it from
their agent's skill list, or asked to "do F1.6". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F1.6`. It gets the CyberTravels code as it stood at
   the end of F1.6 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/grc/guardrail-specification`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/grc/guardrail-specification`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F1.6 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Pick one outcome guardrail your programme has agreed and specify its metric, threshold, source and cadence precisely enough that someone could dispute the result. If you cannot, say so in the coverage report rather than counting it.

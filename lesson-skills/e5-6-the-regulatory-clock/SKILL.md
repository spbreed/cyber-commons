---
name: e5-6-the-regulatory-clock
description: >-
  E5.6 — The regulatory clock. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E5.6, runs it, and reports what happened. Use when working through lesson E5.6 or when asked to do E5.6.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E5.6
  commons-audit: response/regulatory-clock-check
...
The learner wants to do lesson **E5.6**: they typed `/e5-6-the-regulatory-clock`, picked it from
their agent's skill list, or asked to "do E5.6". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E5.6`. It gets the CyberTravels code as it stood at
   the end of E5.6 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/response/regulatory-clock-check`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/response/regulatory-clock-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E5.6 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Build your shortest-clock register: every obligation, its deadline, and who notifies. Then check whether your runbook starts the clock at awareness or at confirmation. The gap between those two is often more than a day.

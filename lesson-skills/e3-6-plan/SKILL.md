...
name: e3-6-plan
description: >-
  E3.6 — Plan, then replan. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E3.6, runs it, and reports what happened. Use when working through lesson E3.6 or when asked to do E3.6.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E3.6
  commons-audit: secops/investigation-replan-trace
...
The learner wants to do lesson **E3.6**: they typed `/e3-6-plan`, picked it from
their agent's skill list, or asked to "do E3.6". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E3.6`. It gets the CyberTravels code as it stood at
   the end of E3.6 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/secops/investigation-replan-trace`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/secops/investigation-replan-trace`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E3.6 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Add evidence that refutes the final hypothesis too. An investigation that cannot end undecided is not investigating.

---
name: e5-5-proposing-the-policy-change
description: >-
  E5.5 — Proposing the policy change. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E5.5, runs it, and reports what happened. Use when working through lesson E5.5 or when asked to do E5.5.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E5.5
  commons-audit: grc/policy-change-proposal
...
The learner wants to do lesson **E5.5**: they typed `/e5-5-proposing-the-policy-change`, picked it from
their agent's skill list, or asked to "do E5.5". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E5.5`. It gets the CyberTravels code as it stood at
   the end of E5.5 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/grc/policy-change-proposal`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/grc/policy-change-proposal`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E5.5 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Write the fourth clause — the one your organisation would refuse. A proposal whose every line is easy did not come from a real incident.

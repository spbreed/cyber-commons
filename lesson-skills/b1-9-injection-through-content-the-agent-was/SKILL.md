---
name: b1-9-injection-through-content-the-agent-was
description: >-
  B1.9 — Injection through content the agent was asked to read. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B1.9, runs it, and reports what happened. Use when working through lesson B1.9 or when asked to do B1.9.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B1.9
  commons-audit: threats/content-derived-privilege-check
...
The learner wants to do lesson **B1.9**: they typed `/b1-9-injection-through-content-the-agent-was`, picked it from
their agent's skill list, or asked to "do B1.9". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B1.9`. It gets the CyberTravels code as it stood at
   the end of B1.9 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/threats/content-derived-privilege-check`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/threats/content-derived-privilege-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B1.9 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** List every place your CI reacts to something the pipeline can produce — comments, labels, branch names, commit trailers. Each one promotes an innocuous tool into a privileged one, without anyone editing the pipeline.

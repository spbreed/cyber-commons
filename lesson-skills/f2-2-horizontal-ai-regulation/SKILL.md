---
name: f2-2-horizontal-ai-regulation
description: >-
  F2.2 — Horizontal AI regulation. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F2.2, runs it, and reports what happened. Use when working through lesson F2.2 or when asked to do F2.2.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F2.2
  commons-audit: regulatory/horizontal-requirement-to-control
...
The learner wants to do lesson **F2.2**: they typed `/f2-2-horizontal-ai-regulation`, picked it from
their agent's skill list, or asked to "do F2.2". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F2.2`. It gets the CyberTravels code as it stood at
   the end of F2.2 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/regulatory/horizontal-requirement-to-control`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/regulatory/horizontal-requirement-to-control`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F2.2 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Take one clause your programme claims to satisfy and trace it to an artefact with a date. If the trail ends at a policy document, the clause is ticked and undefended.

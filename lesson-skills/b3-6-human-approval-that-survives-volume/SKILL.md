---
name: b3-6-human-approval-that-survives-volume
description: >-
  B3.6 — Human approval that survives volume. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B3.6, runs it, and reports what happened. Use when working through lesson B3.6 or when asked to do B3.6.
license: Apache-2.0
...
The learner wants to do lesson **B3.6**: they typed `/b3-6-human-approval-that-survives-volume`, picked it from
their agent's skill list, or asked to "do B3.6". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B3.6`. It gets the CyberTravels code as it stood at
   the end of B3.6 into `work/cybertravels`, shows what this lesson added,
...
3. **The audit.**
   - The audit, `skills/architecture/blast-radius-review`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B3.6 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Count how many approvals your agents generate daily and compare it with 25. If you are above it, decide which actions are reversible enough to be handled by policy instead — that list is usually most of them.

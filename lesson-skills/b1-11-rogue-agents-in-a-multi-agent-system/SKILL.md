---
name: b1-11-rogue-agents-in-a-multi-agent-system
description: >-
  B1.11 — Rogue agents in a multi-agent system. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of B1.11, runs it, and reports what happened. Use when working through lesson B1.11 or when asked to do B1.11.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: B1.11
  commons-audit: threats/agent-registry-gap-check
...
The learner wants to do lesson **B1.11**: they typed `/b1-11-rogue-agents-in-a-multi-agent-system`, picked it from
their agent's skill list, or asked to "do B1.11". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py B1.11`. It gets the CyberTravels code as it stood at
   the end of B1.11 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/threats/agent-registry-gap-check`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/threats/agent-registry-gap-check`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py B1.11 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Ask how your orchestrator decides which agents may receive work. If the answer is a config list or service discovery, write down what would have to be true for an extra entry to be noticed.

---
name: e5-4-validating-the-fix
description: >-
  E5.4 — Validating the fix. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of E5.4, runs it, and reports what happened. Use when working through lesson E5.4 or when asked to do E5.4.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: E5.4
  commons-audit: response/kci-fix-validation
...
The learner wants to do lesson **E5.4**: they typed `/e5-4-validating-the-fix`, picked it from
their agent's skill list, or asked to "do E5.4". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py E5.4`. It gets the CyberTravels code as it stood at
   the end of E5.4 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/response/kci-fix-validation`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/response/kci-fix-validation`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py E5.4 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Add a sixth indicator that regressed — better than target before, worse after. The report has nowhere to put it yet.

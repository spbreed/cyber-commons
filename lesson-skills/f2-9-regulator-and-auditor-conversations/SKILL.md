---
name: f2-9-regulator-and-auditor-conversations
description: >-
  F2.9 — Regulator and auditor conversations. Do this lesson: explains the idea in plain words, gets the CyberTravels code as it stood at the end of F2.9, runs it, and reports what happened. Use when working through lesson F2.9 or when asked to do F2.9.
license: Apache-2.0
allowed-tools: Read, Glob, Bash
metadata:
  commons-lesson: F2.9
  commons-audit: regulatory/assurance-conversation-prep
...
The learner wants to do lesson **F2.9**: they typed `/f2-9-regulator-and-auditor-conversations`, picked it from
their agent's skill list, or asked to "do F2.9". One skill is one lesson, and
...
2. **Run the lesson.** From the repository root:
   `PY scripts/lesson.py F2.9`. It gets the CyberTravels code as it stood at
   the end of F2.9 into `work/cybertravels`, shows what this lesson added,
   runs the tests that exist at this point, and runs the audit
   (`skills/regulatory/assurance-conversation-prep`). It ends with a READBACK.
3. **The audit.**
   - The audit, `skills/regulatory/assurance-conversation-prep`, **asks you a question**. You are the model. The harness prints the audit's task; answer it yourself, following the audit skill's procedure, with one JSON object written to `work/answer.json`, then run `PY scripts/lesson.py F2.9 --answer work/answer.json`.
...
4. **Say what happened.** Read the READBACK to the learner in your own words,
   ...
   **Read this next**. Use only what it printed. If it says a step was skipped
   ...
   not a pass.
5. **Give them the exercise.** Generate this script from your own live control state rather than writing it. If it cannot be generated, your coverage number is being assembled by hand for each meeting — which is why it differs between meetings.
